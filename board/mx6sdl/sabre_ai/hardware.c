...
void usbEnableVbus(usb_module_t * port)
{
    ...
    case Host1:
        ...
        gpio_dir_config(HW_GPIO3, 30, GPIO_GDIR_OUTPUT);
        gpio_write_data(HW_GPIO3, 30, GPIO_HIGH_LEVEL);
#endif
...
#endif
#ifdef BOARD_SMART_DEVICE
        gpio_write_data(HW_GPIO3, 30, GPIO_LOW_LEVEL);
#endif
    case Host2:
#ifdef BOARD_EVB
...
void imx_enet_iomux(void)
{
    ...
    max7310_set_gpio_output(1, 3, GPIO_LOW_LEVEL);  //expander b, io3
    /* Select ALT5 mode of GPIO_19 for GPIO4_5 - RGMII_INT_B */
    /* active low input */
