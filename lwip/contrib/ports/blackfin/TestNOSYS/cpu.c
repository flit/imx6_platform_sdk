...
{
	*pTIMER2_CONFIG = OUT_DIS | PULSE_HI | PWM_OUT;	// Single PWM Pulse, no Output, no interrupt
	*pTIMER2_WIDTH = (CPU_SCLK/10000)*ticks;		// 0.1ms at System Clock
	*pTIMER_ENABLE = TIMEN2;						// Enable Timer

	while (*pTIMER_STATUS & TRUN2);					// Wait the end
}
