...
# Separate assembler sources into .s and .S lists.
ASM_s_SOURCES = $(filter %.s,$(ASM_SOURCES))
ASM_S_SOURCES = $(filter %.S,$(ASM_SOURCES))

# Convert sources to objects.
OBJECTS_C := $(addprefix $(OBJS_ROOT)/,$(notdir $(C_SOURCES:.c=.o)))
OBJECTS_CXX := $(addprefix $(OBJS_ROOT)/,$(notdir $(CXX_SOURCES:.c=.o)))
OBJECTS_ASM := $(addprefix $(OBJS_ROOT)/,$(notdir $(ASM_s_SOURCES:.s=.o)))
OBJECTS_ASM_S := $(addprefix $(OBJS_ROOT)/,$(notdir $(ASM_S_SOURCES:.S=.o)))
...
# to the library.
.PHONY: all
all: $(SUBDIRS) $(OBJS_ROOT) $(OBJECTS_ALL) $(LIBS) $(APP) $(TARGET_LIB)
...
#	@echo Compiling $(addprefix $(MODULE_REL_DIR)$(MODULE)/,$<)
	@echo Compiling $<
	@$(CC) $(CFLAGS) $(SYSTEM_INC) $(C_INCLUDES) $(C_DEFINES) -MMD -MF $(OBJS_ROOT)/$*.d -MP -o $@ -c $<
