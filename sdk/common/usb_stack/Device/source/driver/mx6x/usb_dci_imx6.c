 ...
 *****************************************************************************/
static usb_status_t usbd_mx28_dev_init(uint_8 controller_ID);
...
)
{
    ...
    // Initialize hardware
    status = usbd_mx28_dev_init(controller_ID);
    if (status != USB_OK)
    {
        return status;
    }
...
static usb_status_t usb_set_device_mode(uint_8 controller_ID)
{
    ...
    reg &= ~0x3;
    reg |= BV_USBCTRL_USBMODE_CM__DEVICE;
    ...
    while (((readl(&usbotg[controller_ID]->usbmode) & 0x3) != BV_USBCTRL_USBMODE_CM__DEVICE) && (--count)) ;
...
//	printf("endpctrl is ep[%d]=0x%x, tempj is 0x%x\n", endpt_number, readl(&usbotg->endptctrl[endpt_number]), temp);
	...
	if (direction) {
		if (endpt_number)
			temp |= BM_USBC_UOG_ENDPTCTRLn_TXR;
		temp |= BM_USBC_UOG_ENDPTCTRLn_TXE;
		temp |= ((unsigned int)(ep_type)
				<< BP_USBC_UOG_ENDPTCTRLn_TXT);
	} else {
		if (endpt_number)
			temp |= BM_USBC_UOG_ENDPTCTRLn_RXR;
		temp |= BM_USBC_UOG_ENDPTCTRLn_RXE;
		temp |= ((unsigned int)(ep_type)
				<< BP_USBC_UOG_ENDPTCTRLn_RXT);
	}
