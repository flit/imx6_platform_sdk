			...
			if (fat == FS_FAT12) {
				...
				do {
					...
					if (fat == FS_FAT16) {
						...
						/* todo AI: improve algorithm for counting free 
						 * clusters.
						 * FAT32 File System Specifications:
						 * Any other value(than 0xFFFFFFFF) can be used, but is 
						 * not necessarily correct. It should be range checked 
						 * at least to make sure it is <= volume cluster count.
						 */
						/* set a limit to number of free clusters(workaround 
						 * for some usb sticks)
						 */
						break;
						//printf("counting clusters %d          \r", clst);
					}
				} while (--clst);
			}
			(*fatfs)->free_clust = n;
