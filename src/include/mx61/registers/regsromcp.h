...
/*!
 * @brief HW_ROMC_ROMPATCHD - ROMC Data Registers
 *
 * The ROMC data registers (ROMC_ROMPATCHD7 through ROMC_ROMPATCHD0) store
 * the data to use for the 8 1-word data fix events. Each register is
 * associated with an address comparator (7 through 0). When a data fixing
 * event occurs, the value in the data register corresponding to the
 * comparator that has the address match is put on the romc_hrdata[31:0]
 * bus until romc_hready is asserted by the ROM controller to terminate the
 * access. A MUX external to the ROMC will select this data over that of
 * romc_hrdata[31:0] in returning read data to the ARM core. The selection
 * is done with the control bus rompatch_romc_hrdata_ovr[1:0] with both
 * bits asserted by the ROMC.  If more than one address comparators match, the highest-numbered one
 * takes precedence, and the value in corresponding data register is used
 ...
 *
 * Data Fix Registers - Stores the data used for 1-word data fix
 * operations.  The values stored within these registers do not affect the writes to
 * the memory system. They are selected over the read data from ROM
 * when a data fix event occurs.  If any part of the 1-word data fix is read, then the entire word
 * is                                 replaced. Therefore, a byte or half-word read will cause the
 * ROMC to                                 replace the entire word. The word is word address
 * aligned.
 */
...
/*!
 * @brief HW_ROMC_ROMPATCHCNTL - ROMC Control Register
 *
 * The ROMC control register (ROMC_ROMPATCHCNTL) contains the block disable
 * bit and the data fix enable bits. The block disable bit provides a means
 * to disable the ROMC data fix and opcode patching functions, even when
 * the address comparators are enabled. The External Boot feature is not
 * affected by this bit. The eight data fix enable bits (0 through 7), when
 * set, assign the associated address comparators to data fix operations  Bits 27 and 22 always read
 * as 1s.
 */
#ifndef __LANGUAGE_ASM__
typedef union
 ...
 * comparators for 1-word data fix or for code patch routine.
 */
...
/*!
 * @brief HW_ROMC_ROMPATCHENH - ROMC Enable Register High
 *
 ...
 * comparator can trigger a opcode patch or data fix event. This
 * implementation of the ROMC only has 16 comparators, therefore
 * ROMC_ROMPATCHENH and the upper half of ROMC_OMPATCHENL are read-only.
 * ROMC_ROMPATCHENL[15:0] are associated with comparators 15 through 0.
 * ROMC_ROMPATCHENLH[31:0] would have been associated with comparators 63
 * through 32.
 */
#ifndef __LANGUAGE_ASM__
typedef union
...
/*!
 * @brief HW_ROMC_ROMPATCHENL - ROMC Enable Register Low
 *
 * The ROMC enable register high (ROMC_ROMPATCHENH) and ROMC                         enable register
 * low (ROMC_ROMPATCHENL) control whether or not the associated                         address
 * comparator can trigger a opcode patch or data fix event. This
 * implementation of the ROMC only has 16 comparators, therefore
 * ROMC_ROMPATCHENH and the upper half of ROMC_ROMPATCHENL are read-only.
 * ROMC_ROMPATCHENL[15:0] are associated with comparators 15 through 0.
 * ROMC_ROMPATCHENLH[31:0] would have been associated with comparators 63
 * through 32.
 */
...
/*!
 * @brief HW_ROMC_ROMPATCHA - ROMC Address Registers
 *
 * The ROMC address registers (ROMC_ROMPATCHA0 through ROMC_ROMPATCHA15)
 * store the memory addresses where opcode patching begins and data fixing
 * occurs. The address registers ROMC_ROMPATCHA0 through ROMC_ROMPATCHA15
 * are each 21 bits wide and dedicated to one 4 Mbyte memory space. Bits 21
 * through 2 are address bits, to be compared with romc_haddr[21:2] for a
 * match; bit 1 is also an address bit used for half word selection. Bit 0
 * is the mode bit (set to 1 for THUMB mode). 1-word data fixing can only
 * be used on the first 8 of the address comparators. ROMC_ROMPATCHA0
 * through ROMC_ROMPATCHA15 are associated each with address comparators 0
 * through 15.
 */
#ifndef __LANGUAGE_ASM__
typedef union
 ...
 * THUMB opcode patch or an ARM opcode patch. If this watchpoint is
 * selected to be a data fix, then this bit is ignored as all data
 * fixes are 1-word data fixes.
 */
 ...
 * watched. All 16 registers can be used for code patch address
 * comparison. Only the first 8 registers can be used for a 1-word data
 * fix address comparison.  Bit 1 is ignored if data fix. Only used in code patch
 */
...
/*!
 * @brief HW_ROMC_ROMPATCHSR - ROMC Status Register
 *
 * The ROMC status register (ROMC_ROMPATCHSR) indicates the current state of
 * the ROMC and the source number of the most recent address comparator
 * event.
 */
#ifndef __LANGUAGE_ASM__
typedef union
 ...
 * comparator which has an address match in the most recent patch event
 * on ROMC AHB. If multiple matches occurred, the highest priority
 ...
 *
 * ROMC AHB Multiple Address Comparator matches Indicator - Indicates
 * that multiple address comparator matches occurred. Writing a 1 to
 * this bit will clear this it.
 */
