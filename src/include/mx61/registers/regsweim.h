...
/*!
 * @brief HW_EIM_CSGCR1 - Chip Select n General Configuration Register                        1
 *
 ...
 *
 * CS Enable. This bit controls the operation of the chip select pin.
 * CSEN is set by a hardware reset for CSGCR0 to allow external boot
 * operation. CSEN is cleared by a hardware reset to CSGCR1-CSGCR5.  Reset value for EIM_CS0GCR1 for
 * CSEN is 1. For EIM_CS1GCR1-CS1GCR5                                 reset value is 0.
 */
 ...
 *
 * Synchronous Write Data. This bit field determine the write accesses
 * mode to the External device of the chip select. The External device
 * should be configured to the same mode as this bit implicates. SWR is
 * cleared by a hardware reset.  Sync. accesses supported only for 16/32 bit port.
 */
 ...
 *
 * Synchronous Read Data. This bit field determine the read accesses
 * mode to the External device of the chip select. The External device
 * should be configured to the same mode as this bit implicates. SRD is
 * cleared by a hardware reset.  Sync. accesses supported only for 16/32 bit port.
 */
 ...
 *
 * Multiplexed Mode. This bit determines the address/data multiplexed
 * mode for asynchronous and synchronous accesses for 8 bit, 16 bit or
 * 32 bit devices (DSZ config. dependent).  The reset value for EIM_CS0GCR1[MUM] = EIM_BOOT[2]. For
 * EIM_CS1GCR1 - EIM_CS5GCR1 the reset value is 0.
 */
 ...
 *
 * Write Fix Latency. This bit field determine if the controller is
 * monitoring the WAIT signal from the External device connected to the
 * chip select (handshake mode - fix or variable data latency) or if it
 * start data transfer according to WWSC field, it only valid in
 * synchronous mode. WFL is cleared by a hardware reset.  When WFL=1 Burst access is terminated on
 * page boundary and resume on                                 the following page according to BL
 * bit field configuration, because                                 WAIT signal is not monitored
 ...
 *
 * Read Fix Latency. This bit field determine if the controller is
 * monitoring the WAIT signal from the External device connected to the
 * chip select (handshake mode - fix or variable data latency) or if it
 * start sampling data according to RWSC field, it only valid in
 * synchronous mode. RFL is cleared by a hardware reset.  When RFL=1 Burst access is terminated on
 * page boundary and resume on                                 the following page according to BL
 * bit field configuration, because                                 WAIT signal is not monitored
 * from the external device.
 */
 ...
 *
 * Configuration Register Enable. This bit indicates CRE memory pin
 * state while executing a memory register set command to PSRAM
 * external device. CRE is cleared by a hardware reset.
 */
 ...
 *
 * Configuration Register Enable Polarity. This bit indicates CRE memory
 * pin assertion state, active-low or active-high, while executing a
 * memory register set command to the external device (PSRAM memory
 * type). CREP is set by a hardware reset.  Whenever PSRAM is connected the CREP value must be
 * correct also                                 for accesses where CRE is disabled.  For Non-PSRAM
 * memory CREP value should be 1.
 */
 ...
 *
 * Burst Length. The BL bit field indicates memory burst length in words
 * (word is defined by the DSZ field) and should be properly
 * initialized for mixed wrap/increment accesses support. Continuous BL
 * value corresponds to continuous burst length setting of the external
 * memory device. For fix memory burst size, type is always wrap. In
 * case not matching wrap boundaries in both the memory (BL field) and
 * Master access on the current address, EIM update address on the
 * external device address bus and regenerates the access.  BL is cleared by a hardware reset.  When
 * APR=1, Page Read Mode is applied, BL determine the number of
 * words within the read page burst. BL is cleared by a hardware reset
 * for EIM_CS0GCR1 - EIM_CS5GCR1.
 */
 ...
 *
 * Write Continuous. The WI bit indicates that write access to the
 * memory are always continuous accesses regardless of the BL field
 * value. WI is cleared by hardware reset.
 */
 ...
 *
 * Burst Clock Divisor. This bit field contains the value used to
 * program the burst clock divisor for BCLK generation. It is used to
 * divide the internal EIMbus frequency. BCD is cleared by a hardware
 * reset.  For other then the mentioned below frequency such as 104 MHz, EIM
 * clock (input clock) should be adjust accordingly.
 */
 ...
 *
 * Burst Clock Start. When SRD=1 or SWR=1,this bit field determines the
 * number of EIM clock cycles delay from start of access before the
 * first rising edge of BCLK is generated.  When BCD=0 value of BCS=0 results in a half clock delay
 * after the                                 start of access. For other values of BCD a one clock
 * delay after the                                 start of access is applied, not an immediate
 * assertion. BCS is                                 cleared by a hardware reset.
 */
 ...
 *
 * Data Port Size. This bit field defines the width of an external
 * device's data port as shown below.  Only async. access supported for 8 bit port.  The reset value
 * for EIM_CS0GCR1[DSZ] = {EIM_BOOT[11],                                 EIM_BOOT[1:0]} EIM_CS0GCR1,
 * DSZ[2] = 0, DSZ[1:0] = EIM_BOOT[1:0]. For EIM_CS1GCR1 - EIM_CS5GCR1, the reset value is
 * 0b001.
 */
 ...
 *
 * Supervisor Protect. This bit prevents accesses to the address range
 * defined by the corresponding chip select when the access is
 * attempted in the User mode. SP is cleared by a hardware reset.
 */
 ...
 *
 * CS Recovery. This bit field, according to the settings shown below,
 * determines the minimum pulse width of CS, OE, and WE control signals
 * before executing a new back to back access to the same chip select.
 * CSREC is cleared by a hardware reset.  The reset value for EIM_CS0GCR1, CSREC[2:1] is
 * EIM_BOOT[9:8], for                                 CSREC[0] is 0 CSREC[2:0] is 0b110. For
 * EIM_CS1GCR1 - EIM_CS5GCR, the reset value is                                 0b000.  Example
 * settings:
 */
 ...
 *
 * Address UnShifted. This bit indicates an unshifted mode for address
 * assertion for the relevant chip select accesses. AUS bit is cleared
 * by hardware reset.  The reset value for EIM_CS0GCR1[AUS] = EIM_BOOT[10]. For
 * EIM_CS1GCR1 - EIM_CS5GCR1, the reset value of AUS is 0.
 */
 ...
 *
 * Gap Between Chip Selects. This bit field, according to the settings
 * shown below, determines the minimum time between end of access to
 * the current chip select and start of access to different chip
 * select. GBC is cleared by a hardware reset.  Example settings:
 */
 ...
 *
 * Write Protect. This bit prevents writes to the address range defined
 * by the corresponding chip select. WP is cleared by a hardware
 * reset.
 */
 ...
 *
 * Page Size. This bit field indicates memory page size in words (word
 * is defined by the DSZ field). PSZ is used when fix latency mode is
 * applied, WFL=1 for sync. write accesses, RFL=1 for sync. Read
 * accesses. When working in fix latency mode WAIT signal from the
 * external device is not being monitored, PSZ is used to determine if
 * page boundary is reached and renewal of access is preformed. This
 * bit field is ignored when sync. Mode is disabled or fix latency mode
 * is not being used for write or read access separately.  It can be valid for both access type,
 * read or write, or only for one                                 type, according to configuration.
...
/*!
 * @brief HW_EIM_CSGCR2 - Chip Select n General Configuration Register                        2
 *
 ...
 *
 * Address hold time - This bit field determine the address hold time
 * after ADV negation when mum = 1 (muxed mode).  When mum = 0 this bit has no effect. For read
 * accesses the field                                 determines when the pads direction will be
 * switched.  Reset value for EIM_CS0GCR2 for ADH is 10. For
 ...
 *
 * Data Acknowledge Poling Start. This bit field determine the starting
 * point of DTACK input signal polling. DAPS is used only in
 * asynchronous single read or write accesses.  Since DTACK is an async. signal the start point of
 * DTACK signal                                 polling is at least 3 cycles after the start of
 * access.  DAPS is cleared by a hardware reset.  Example settings:
 */
 ...
 *
 * Data Acknowledge Enable. This bit indicates external device is using
 * DTACK pin as strobe/terminator of an async. access. DTACK signal may
 * be used only in asynchronous single read (APR=0) or write accesses.
 * DTACK poling start point is set by DAPS bit field. polarity of DTACK
 ...
 *
 * Data Acknowledge Polarity. This bit indicates DTACK memory pin
 * assertion state, active-low or active-high, while executing an async
 * access using DTACK signal from the external device. DAP is cleared
 * by a hardware reset.
 */
 ...
 *
 * Muxed 16 bypass grant. This bit when asserted causes EIM to bypass
 * the grant/ack. arbitration with NFC (only for 16 bit muxed mode
 * accesses).  The reset value for EIM_CS0GCR2[MUX16_BYP_GRANT] = EIM_BOOT[12].
 * For EIM_CS1GCR2 - EIM_CS5GCR2, MUX16_BYP_GRANT reset value is                                 1.
 */
...
/*!
 * @brief HW_EIM_CSRCR1 - Chip Select n Read Configuration Register 1
 *
 ...
 *
 * Read CS Negation. This bit field determines when CS signal is negated
 * during read cycles in asynchronous single mode only (SRD=0 & APR
 * = 0), according to the settings shown below. This bit field is
 * ignored when SRD=1. RCSN is cleared by a hardware reset.  Example settings:
 */
 ...
 *
 * Read CS Assertion. This bit field determines when CS signal is
 * asserted during read cycles (synchronous or asynchronous mode),
 * according to the settings shown below. RCSA is cleared by a hardware
 ...
 *
 * OE Negation. This bit field determines when OE signal is negated
 * during read cycles in asynchronous single mode only (SRD=0 & APR
 * = 0), according to the settings shown below. This bit field is
 * ignored when SRD=1. OEN is cleared by a hardware reset.  Example settings:
 */
 ...
 *
 * OE Assertion. This bit field determines when OE signal are asserted
 * during read cycles (synchronous or asynchronous mode), according to
 * the settings shown below. OEA is cleared by a hardware reset.  In muxed mode OE assertion occurs
 * (OEA + RADVN + RADVA + ADH +1) EIM                                 clock cycles from start of
 * access.  The reset value for EIM_CS0RCR1[OEA] is 0b000 if EIM_BOOT[2] = 0.
 * If EIM_BOOT[2] is 1, the reset value for EIM_CS0RCR1 is 0b010. The
 * reset value of this field for EIM_CS1RCR1 - EIM_CS5RCR1 is                                 0b000.
 * Example settings:
 */
 ...
 *
 * ADV Negation. This bit field determines when ADV signal to memory is
 * negated during read accesses.  When SRD=1 (synchronous read mode), ADV negation occurs according
 * to                                 the following formula: (RADVN + RADVA + BCD + BCS + 1) EIM
 * clock                                 cycles from start of access.  When asynchronous read mode
 * is applied (SRD=0) and RAL=0 ADV negation                                 occurs according to the
 * following formula: (RADVN + RADVA + 1) EIM                                 clock cycles from
 * start of access. RADVN is cleared by a hardware                                 reset.  the reset
 * value for EIM_CS0RCR1[RADVN] = 2. For EIM_CS1RCR1 -                                 EIM_CS5RCR1,
 * the reset value is 0b000.  This field should be configured so ADV negation will occur before
 * the end of access. For ADV negation at the same time with the end of
 * access user should RAL bit.
 */
 ...
 *
 * Read ADV Low. This bit field determine ADV signal negation time. When
 * RAL=1, RADVN bit field is ignored and ADV signal will stay asserted
 * until end of access. When RAL=0 negation of ADV signal is according
 * to RADVN bit field configuration.  The reset value of EIM_CS0RCR1[RAL] = EIM_BOOT[3]. RAL is
 * cleared                                 by a hardware reset for EIM_CS1RCR1 - EIM_CS5RCR1.
 */
 ...
 *
 * ADV Assertion. This bit field determines when ADV signal is asserted
 * for synchronous or asynchronous read modes according to the settings
 * shown below. RADVA is cleared by a hardware reset.  Example settings:
 */
 ...
 *
 * Read Wait State Control. This bit field programs the number of
 * wait-states, according to the settings shown below, for synchronous
 * or asynchronous read access to the external device connected to the
 * chip select.  When SRD=1 and RFL=0, RWSC indicates the number of burst clock (BCLK)
 * cycles from the start of an access, before the controller can start
 * sample data.Since WAIT signal can be asserted one cycle before the
 * first data can be sampled, the controller starts evaluating the WAIT
 * signal state one cycle before, this is referred as handshake mode or
 * variable latency mode.  When SRD=1 and RFL=1, RWSC indicates the number of burst clock (BCLK)
 * cycles from the start of an access, until the external device is
 * ready for data transfer, this is referred as fix latency mode.  When SRD=0, RFL bit is ignored,
 * RWSC indicates the asynchronous                                 access length and the number of
 * EIM clock cycles from the start of                                 access until the external
 * device is ready for data transfer.  RWSC is cleared by a hardware reset.  The reset value for
 * EIM_CS0RCR1[RWSC[4:2]] = EIM_BOOT [7:5]. For                                 {RWSC[5], RWSC[1:0]}
 * the reset value is 0b000 EIM_CS0RCR1, RWSC[5:0] = 0b011100. For CG1RCR1 - CS1RCR5
 * the reset value is 0b000000.  Example settings:
 */
...
/*!
 * @brief HW_EIM_CSRCR2 - Chip Select n Read Configuration Register 2
 *
 ...
 *
 * Read BE Negation. This bit field determines when BE signal is negated
 * during read cycles in asynchronous single mode only (SRD=0 &
 * APR=0), according to the settings shown below. This bit field is
 * ignored when SRD=1. RBEN is cleared by a hardware reset.  Example settings:
 */
 ...
 *
 * Read BE enable. This bit field determines if BE will be asserted
 * during read access.
 */
 ...
 *
 * Read BE Assertion. This bit field determines when BE signal is
 * asserted during read cycles (synchronous or asynchronous mode),
 * according to the settings shown below. RBEA is cleared by a hardware
 * reset.  Example settings:
 */
 ...
 *
 * Read Latency. This bit field indicates cycle latency when executing a
 * synchronous read operation.  The fields holds the feedback clock loop delay in aclk cycle
 * units.  This field is cleared by a hardware reset.
 */
 ...
 *
 * Page Access Time. This bit field is used in Asynchronous Page Read
 * mode only (APR=1). the initial access is set by RWSC as in regular
 * asynchronous mode. the consecutive address assertions width
 * determine by PAT field according to the settings shown below. when
 * APR=0 this field is ignored.  PAT is cleared by a hardware reset for EIM_CS1GCR1 - EIM_CS5GCR1.
 */
 ...
 *
 * Asynchronous Page Read. This bit field determine the asynchronous
 * read mode to the external device. When APR=0, the async. read access
 * is done as single word (where word is defined by the DSZ field).
 * when APR=1, the async. read access executed as page read. page size
 * is according to BL field config., RCSN,RBEN,OEN and RADVN are being
 * ignored.  APR is cleared by a hardware reset for EIM_CS1GCR1 - EIM_CS5GCR1.  SRD=0 and MUM=0 must
 * apply when APR=1
 */
...
/*!
 * @brief HW_EIM_CSWCR1 - Chip Select n Write Configuration Register                        1
 *
 ...
 *
 * Write CS Negation. This bit field determines when CS signal is
 * negated during write cycles in asynchronous mode only (SWR=0),
 * according to the settings shown below. This bit field is ignored
 ...
 *
 * Write CS Assertion. This bit field determines when CS signal is
 * asserted during write cycles (synchronous or asynchronous mode),
 * according to the settings shown below.this bit field is ignored when
 * executing a read access to the external device. WCSA is cleared by a
 ...
 *
 * WE Negation. This bit field determines when WE signal is negated
 * during write cycles in asynchronous mode only (SWR=0), according to
 * the settings shown below. This bit field is ignored when SWR=1. WEN
 * is cleared by a hardware reset.  Reset value for EIM_CS0WCR for WEN is 2. For EIM_CS1WCR -
 * EIM_CS5WCR reset value is 000.  Example settings:
 */
 ...
 *
 * WE Assertion. This bit field determines when WE signal is asserted
 * during write cycles (synchronous or asynchronous mode), according to
 * the settings shown below. This bit field is ignored when executing a
 * read access to the external device. WEA is cleared by a hardware
 * reset.  Reset value for EIM_CS0WCR for WEA is 2. For EIM_CS1WCR -
 * EIM_CS5WCR reset value is 000.  Example settings:
 */
 ...
 *
 * BE[3:0] Negation. This bit field determines when BE[3:0] bus signal
 * is negated during write cycles in async. mode only (SWR=0),
 * according to the settings shown below. This bit field is ignored
 * when SWR=1. BEN is cleared by a hardware reset.  Reset value for EIM_CS0WCR for WBEN is 2. For
 * EIM_CS1WCR -                                 EIM_CS5WCR reset value is 000.  Example settings:
 * 000 0 EIM clock cycles between end of access and WE negation  001 1 EIM clock cycles between end
 * of access and WE negation  010 2 EIM clock cycles between end of access and WE negation  111 7
 * EIM clock cycles between end of access and WE negation
 */
 ...
 *
 * BE Assertion. This bit field determines when BE signal is asserted
 * during write cycles in async. mode only (SWR=0), according to the
 * settings shown below. BEA is cleared by a hardware reset.  Reset value for EIM_CS0WCR for WBEA is
 * 2. For EIM_CS1WCR -                                 EIM_CS5WCR reset value is 000.  Example
 * settings:
 */
 ...
 *
 * ADV Negation. This bit field determines when ADV signal to memory is
 * negated during write accesses.  When SWR=1 (synchronous write mode), ADV negation occurs
 * according to                                 the following formula: (WADVN + WADVA + BCD + BCS +
 * 1) EIM clock                                 cycles.  When asynchronous read mode is applied
 * (SWR=0) ADV negation occurs                                 according to the following formula:
 * (WADVN + WADVA + 1) EIM clock                                 cycles.  Reset value for EIM_CS0WCR
 * for WADVN is 2. For EIM_CS1WCR -                                 EIM_CS5WCR reset value is 000.
 * This field should be configured so ADV negation will occur before
 * the end of access. For ADV negation at the same time as the end of
 * access, S/W should set the WAL bit.
 */
 ...
 *
 * ADV Assertion. This bit field determines when ADV signal is asserted
 * for synchronous or asynchronous write modes according to the
 * settings shown below. WADVA is cleared by a hardware reset.  Example settings:
 */
 ...
 *
 * Write Wait State Control. This bit field programs the number of
 * wait-states, according to the settings shown below, for synchronous
 * or asynchronous write access to the external device connected to the
 * chip select.  When SWR=1 and WFL=0, WWSC indicates the number of burst clock (BCLK)
 * cycles from the start of an access, before the memory can sample the
 * first data.Since WAIT signal can be asserted one cycle before the
 * first data can be sampled, the controller starts evaluating the WAIT
 * signal state one cycle before, this is referred as handshake mode or
 * variable latency mode.  When SWR=1 and WFL=1, WWSC indicates the number of burst clock (BCLK)
 * cycles from the start of an access, until the external device is
 * ready for data transfer, this is referred as fix latency mode.  When SWR=0, WFL bit is ignored,
 * WWSC indicates the asynchronous                                 access length and the number of
 * EIM clock cycles from the start of                                 access until the external
 * device is ready for data transfer.  WWSC is cleared by a hardware reset.  The reset value for
 * EIM_CS0WCR1[WWSC[4:2]] = EIM_BOOT [7:5],                                 {WWSC[5], WWSC[1:0]} =
 * 0b000 EIM_CS0WCR1, WWSC[5:0] = 0b011100. For EIM_CS1WCR1 - EIM_CS5WCR1, the
 * reset value of this field is 0b000000.  Example settings:
 */
 ...
 *
 * Write Byte Enable Disable. When asserted this bit prevent from
 * IPP_DO_BE_B[x] to be asserted during write accesses.This bit is
 ...
 *
 * Write ADV Low. This bit field determine ADV signal negation time in
 * write accesses. When WAL=1, WADVN bit field is ignored and ADV
 * signal will stay asserted until end of access. When WAL=0 negation
 * of ADV signal is according to WADVN bit field configuration.  The reset value of CS0WCR1[WAL] =
 * EIM_BOOT[3]. This field is                                 cleared by a hardware reset for
 * CS1WCR1 - CS5WCR1.
 */
...
/*!
 * @brief HW_EIM_CSWCR2 - Chip Select n Write Configuration Register                        2
 *
 ...
 *
 * Write Burst Clock Divisor Decrement. If this bit is asserted and BCD
 * value is 0 sync. write access will be preformed as if BCD value is
 * 1.When this bit is negated or BCD value is not 0 this bit has no
 * affect.  This bit is cleared by hardware reset.
 */
