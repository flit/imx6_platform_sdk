...
typedef union
{
    ...
    struct
    {
        unsigned RUN : 1; //!< When this bit is set by software, the eLCDIF will start fetching data in either the DMA mode or the bus master mode and sending it across the interface. This bit must remain set for all the time the block is in operation.
        unsigned DATA_FORMAT_24_BIT : 1; //!< Used only when WORD_LENGTH = 3, i.e. 24-bit. Note that this applies to both packed and unpacked 24-bit data.
        unsigned DATA_FORMAT_18_BIT : 1; //!< Used only when WORD_LENGTH = 2, i.e. 18-bit.
        unsigned DATA_FORMAT_16_BIT : 1; //!< When this bit is 1 and WORD_LENGTH = 0, it implies that the the 16-bit data is in ARGB555 format. When this bit is 0 and WORD_LENGTH = 0, it implies that the 16-bit data is in RGB565 format. When WORD_LENGTH is not 0, this bit is a dont care.
        unsigned RESERVED0 : 1; //!< Reserved bits. Write as 0.
        unsigned MASTER : 1; //!< Set this bit to make the eLCDIF act as a bus master. If this bit is reset, the eLCDIF will act in its traditional DMA slave mode.
        unsigned ENABLE_PXP_HANDSHAKE : 1; //!< If this bit is set and LCDIF_MASTER bit is set, the eLCDIF will act as bus master and the handshake mechanism between eLCDIF and ePXP will be turned on. If LCDIF_MASTER bit is not set, this bit becomes a don't care.
        unsigned RGB_TO_YCBCR422_CSC : 1; //!< Set this bit to 1 to enable conversion from RGB to YCbCr colorspace. See the LCDIF_CSC_ registers for further details.
        unsigned WORD_LENGTH : 2; //!< Input data format.
        unsigned LCD_DATABUS_WIDTH : 2; //!< LCD Data bus transfer width.
        unsigned CSC_DATA_SWIZZLE : 2; //!< This field specifies how to swap the bytes after the data has been converted into an internal representation of 24 bits per pixel and before it is transmitted over the LCD interface bus. The data is always transmitted with the least significant byte/hword (half word) first after the swizzle takes place. So, INPUT_DATA_SWIZZLE takes place first on the incoming data, and then CSC_DATA_SWIZZLE is applied. The swizzle function is independent of the WORD_LENGTH or the LCD_DATABUS_WIDTH fields. If RGB_TO_YCRCB422_CSC bit is set, the swizzle occurs on the Y, Cb, Cr values. The supported swizzle configurations are:
        unsigned INPUT_DATA_SWIZZLE : 2; //!< This field specifies how to swap the bytes fetched by the bus master interface. The swizzle function is independent of the WORD_LENGTH bit. The supported swizzle configurations are:
        unsigned DATA_SELECT : 1; //!< Command Mode polarity bit. This bit should only be changed when RUN is 0.
        unsigned DOTCLK_MODE : 1; //!< Set this bit to 1 to make the hardware go into the DOTCLK mode, i.e. VSYNC/HSYNC/DOTCLK/ENABLE interface mode. ENABLE is optional, selected by the ENABLE_PRESENT bit. Toggle this bit from 1 to 0 to make the hardware go out of DOTCLK mode after completing all data transfer and deasserting the RUN bit.
        unsigned VSYNC_MODE : 1; //!< Setting this bit to 1 will make the eLCDIF hardware go into VSYNC mode. WAIT_FOR_VSYNC_EDGE can be used only if this bit is set. If VSYNC signal is required to be an output from the block, SYNC_SIGNALS_ON bit in LCDIF_VDCTRL4 register must be set.
        unsigned BYPASS_COUNT : 1; //!< When this bit is 0, it means that eLCDIF will stop the block operation and turn off the RUN bit after the amount of data indicated by the LCDIF_TRANSFER_COUNT register has been transferred out. When this bit is set to 1, the block will continue normal operation indefinitely until it is told to stop. This bit must be 0 in MPU and VSYNC modes, and must be 1 in DOTCLK and DVI modes of operation.
        unsigned DVI_MODE : 1; //!< Set this bit to 1 to get into theITU-R BT.656 digital video interface mode. Toggle this bit from 1 to 0 to make the hardware go out of DVI mode after completing all data transfer, deasserting the RUN bit and toggling the dma_end_cmd signal.
        unsigned SHIFT_NUM_BITS : 5; //!< The data to be transmitted is shifted left or right by this number of bits.
        unsigned DATA_SHIFT_DIR : 1; //!< Use this bit to determine the direction of shift of transmit data. In the DVI mode, it works only on the active data, not on the timing codes and ancillary data.
        unsigned WAIT_FOR_VSYNC_EDGE : 1; //!< Setting this bit to 1 will make the hardware wait for the triggering VSYNC edge before starting write transfers to the LCD. Used only in the VSYNC mode of operation.
        unsigned READ_WRITEB : 1; //!< By default, eLCDIF is in the write mode. Setting this bit to 1 will make the hardware go into 6800/8080 MPU read mode. Make sure that DMA operation is selected (LCDIF_MASTER bit is 0) when performing read operations.
        unsigned YCBCR422_INPUT : 1; //!< Zero implies input data is in RGB color space. One implies input data is in YCbCr 4:2:2 format, such that YCbYCr are packed in a 32-bit word. It also means that there are 2 pixels in 4 bytes. If this bit is set, software should program the H_COUNT field in the TRANSFER_COUNT register to the total number of pixels that will have to be fetched by the eLCDIF block per line and the BYTE_PACKING_FORMAT should be 0xF. The WORD_LENGTH does not matter in this case.
        ...
        unsigned SFTRST : 1; //!< This bit must be set to zero to enable normal operation of the eLCDIF. When set to one, it forces a block level reset.
    } B;
} hw_elcdif_ctrl_t;
#endif
...
#define HW_GPMI_CTRL0_SET(v)    ((*(volatile reg32_t *) HW_ELCDIF_CTRL_SET_ADDR) = (v))
...
/* --- Register HW_ELCDIF_CTRL, field RUN[0:0] (RW)
 *
...
#define BP_ELCDIF_CTRL_RUN      (0)
#define BM_ELCDIF_CTRL_RUN      (0x00000001)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_RUN(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CTRL_RUN)
#else
#define BF_ELCDIF_CTRL_RUN(v)   (((v) << 0) & BM_ELCDIF_CTRL_RUN)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field DATA_FORMAT_24_BIT[1:1] (RW)
 *
...
#define BP_ELCDIF_CTRL_DATA_FORMAT_24_BIT      (1)
#define BM_ELCDIF_CTRL_DATA_FORMAT_24_BIT      (0x00000002)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_DATA_FORMAT_24_BIT(v)   ((((reg32_t) v) << 1) & BM_ELCDIF_CTRL_DATA_FORMAT_24_BIT)
#else
#define BF_ELCDIF_CTRL_DATA_FORMAT_24_BIT(v)   (((v) << 1) & BM_ELCDIF_CTRL_DATA_FORMAT_24_BIT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field DATA_FORMAT_18_BIT[2:2] (RW)
 *
...
#define BP_ELCDIF_CTRL_DATA_FORMAT_18_BIT      (2)
#define BM_ELCDIF_CTRL_DATA_FORMAT_18_BIT      (0x00000004)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_DATA_FORMAT_18_BIT(v)   ((((reg32_t) v) << 2) & BM_ELCDIF_CTRL_DATA_FORMAT_18_BIT)
#else
#define BF_ELCDIF_CTRL_DATA_FORMAT_18_BIT(v)   (((v) << 2) & BM_ELCDIF_CTRL_DATA_FORMAT_18_BIT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field DATA_FORMAT_16_BIT[3:3] (RW)
 *
...
#define BP_ELCDIF_CTRL_DATA_FORMAT_16_BIT      (3)
#define BM_ELCDIF_CTRL_DATA_FORMAT_16_BIT      (0x00000008)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_DATA_FORMAT_16_BIT(v)   ((((reg32_t) v) << 3) & BM_ELCDIF_CTRL_DATA_FORMAT_16_BIT)
#else
#define BF_ELCDIF_CTRL_DATA_FORMAT_16_BIT(v)   (((v) << 3) & BM_ELCDIF_CTRL_DATA_FORMAT_16_BIT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field MASTER[5:5] (RW)
 *
...
#define BP_ELCDIF_CTRL_MASTER      (5)
#define BM_ELCDIF_CTRL_MASTER      (0x00000020)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_MASTER(v)   ((((reg32_t) v) << 5) & BM_ELCDIF_CTRL_MASTER)
#else
#define BF_ELCDIF_CTRL_MASTER(v)   (((v) << 5) & BM_ELCDIF_CTRL_MASTER)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field ENABLE_PXP_HANDSHAKE[6:6] (RW)
 *
...
#define BP_ELCDIF_CTRL_ENABLE_PXP_HANDSHAKE      (6)
#define BM_ELCDIF_CTRL_ENABLE_PXP_HANDSHAKE      (0x00000040)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_ENABLE_PXP_HANDSHAKE(v)   ((((reg32_t) v) << 6) & BM_ELCDIF_CTRL_ENABLE_PXP_HANDSHAKE)
#else
#define BF_ELCDIF_CTRL_ENABLE_PXP_HANDSHAKE(v)   (((v) << 6) & BM_ELCDIF_CTRL_ENABLE_PXP_HANDSHAKE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field RGB_TO_YCBCR422_CSC[7:7] (RW)
 *
 * Set this bit to 1 to enable conversion from RGB to YCbCr colorspace. See the LCDIF_CSC_ registers
 * for further details.
 */

#define BP_ELCDIF_CTRL_RGB_TO_YCBCR422_CSC      (7)
#define BM_ELCDIF_CTRL_RGB_TO_YCBCR422_CSC      (0x00000080)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_RGB_TO_YCBCR422_CSC(v)   ((((reg32_t) v) << 7) & BM_ELCDIF_CTRL_RGB_TO_YCBCR422_CSC)
#else
#define BF_ELCDIF_CTRL_RGB_TO_YCBCR422_CSC(v)   (((v) << 7) & BM_ELCDIF_CTRL_RGB_TO_YCBCR422_CSC)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_WORD_LENGTH(v)   ((((reg32_t) v) << 8) & BM_ELCDIF_CTRL_WORD_LENGTH)
#else
#define BF_ELCDIF_CTRL_WORD_LENGTH(v)   (((v) << 8) & BM_ELCDIF_CTRL_WORD_LENGTH)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_LCD_DATABUS_WIDTH(v)   ((((reg32_t) v) << 10) & BM_ELCDIF_CTRL_LCD_DATABUS_WIDTH)
#else
#define BF_ELCDIF_CTRL_LCD_DATABUS_WIDTH(v)   (((v) << 10) & BM_ELCDIF_CTRL_LCD_DATABUS_WIDTH)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_CSC_DATA_SWIZZLE(v)   ((((reg32_t) v) << 12) & BM_ELCDIF_CTRL_CSC_DATA_SWIZZLE)
#else
#define BF_ELCDIF_CTRL_CSC_DATA_SWIZZLE(v)   (((v) << 12) & BM_ELCDIF_CTRL_CSC_DATA_SWIZZLE)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_INPUT_DATA_SWIZZLE(v)   ((((reg32_t) v) << 14) & BM_ELCDIF_CTRL_INPUT_DATA_SWIZZLE)
#else
#define BF_ELCDIF_CTRL_INPUT_DATA_SWIZZLE(v)   (((v) << 14) & BM_ELCDIF_CTRL_INPUT_DATA_SWIZZLE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field DATA_SELECT[16:16] (RW)
 *
...
#define BP_ELCDIF_CTRL_DATA_SELECT      (16)
#define BM_ELCDIF_CTRL_DATA_SELECT      (0x00010000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_DATA_SELECT(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CTRL_DATA_SELECT)
#else
#define BF_ELCDIF_CTRL_DATA_SELECT(v)   (((v) << 16) & BM_ELCDIF_CTRL_DATA_SELECT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field DOTCLK_MODE[17:17] (RW)
 *
...
#define BP_ELCDIF_CTRL_DOTCLK_MODE      (17)
#define BM_ELCDIF_CTRL_DOTCLK_MODE      (0x00020000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_DOTCLK_MODE(v)   ((((reg32_t) v) << 17) & BM_ELCDIF_CTRL_DOTCLK_MODE)
#else
#define BF_ELCDIF_CTRL_DOTCLK_MODE(v)   (((v) << 17) & BM_ELCDIF_CTRL_DOTCLK_MODE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field VSYNC_MODE[18:18] (RW)
 *
...
#define BP_ELCDIF_CTRL_VSYNC_MODE      (18)
#define BM_ELCDIF_CTRL_VSYNC_MODE      (0x00040000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_VSYNC_MODE(v)   ((((reg32_t) v) << 18) & BM_ELCDIF_CTRL_VSYNC_MODE)
#else
#define BF_ELCDIF_CTRL_VSYNC_MODE(v)   (((v) << 18) & BM_ELCDIF_CTRL_VSYNC_MODE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field BYPASS_COUNT[19:19] (RW)
 *
...
#define BP_ELCDIF_CTRL_BYPASS_COUNT      (19)
#define BM_ELCDIF_CTRL_BYPASS_COUNT      (0x00080000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_BYPASS_COUNT(v)   ((((reg32_t) v) << 19) & BM_ELCDIF_CTRL_BYPASS_COUNT)
#else
#define BF_ELCDIF_CTRL_BYPASS_COUNT(v)   (((v) << 19) & BM_ELCDIF_CTRL_BYPASS_COUNT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field DVI_MODE[20:20] (RW)
 *
...
#define BP_ELCDIF_CTRL_DVI_MODE      (20)
#define BM_ELCDIF_CTRL_DVI_MODE      (0x00100000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_DVI_MODE(v)   ((((reg32_t) v) << 20) & BM_ELCDIF_CTRL_DVI_MODE)
#else
#define BF_ELCDIF_CTRL_DVI_MODE(v)   (((v) << 20) & BM_ELCDIF_CTRL_DVI_MODE)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_SHIFT_NUM_BITS(v)   ((((reg32_t) v) << 21) & BM_ELCDIF_CTRL_SHIFT_NUM_BITS)
#else
#define BF_ELCDIF_CTRL_SHIFT_NUM_BITS(v)   (((v) << 21) & BM_ELCDIF_CTRL_SHIFT_NUM_BITS)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field DATA_SHIFT_DIR[26:26] (RW)
 *
...
#define BP_ELCDIF_CTRL_DATA_SHIFT_DIR      (26)
#define BM_ELCDIF_CTRL_DATA_SHIFT_DIR      (0x04000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_DATA_SHIFT_DIR(v)   ((((reg32_t) v) << 26) & BM_ELCDIF_CTRL_DATA_SHIFT_DIR)
#else
#define BF_ELCDIF_CTRL_DATA_SHIFT_DIR(v)   (((v) << 26) & BM_ELCDIF_CTRL_DATA_SHIFT_DIR)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field WAIT_FOR_VSYNC_EDGE[27:27] (RW)
 *
...
#define BP_ELCDIF_CTRL_WAIT_FOR_VSYNC_EDGE      (27)
#define BM_ELCDIF_CTRL_WAIT_FOR_VSYNC_EDGE      (0x08000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_WAIT_FOR_VSYNC_EDGE(v)   ((((reg32_t) v) << 27) & BM_ELCDIF_CTRL_WAIT_FOR_VSYNC_EDGE)
#else
#define BF_ELCDIF_CTRL_WAIT_FOR_VSYNC_EDGE(v)   (((v) << 27) & BM_ELCDIF_CTRL_WAIT_FOR_VSYNC_EDGE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field READ_WRITEB[28:28] (RW)
 *
...
#define BP_ELCDIF_CTRL_READ_WRITEB      (28)
#define BM_ELCDIF_CTRL_READ_WRITEB      (0x10000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_READ_WRITEB(v)   ((((reg32_t) v) << 28) & BM_ELCDIF_CTRL_READ_WRITEB)
#else
#define BF_ELCDIF_CTRL_READ_WRITEB(v)   (((v) << 28) & BM_ELCDIF_CTRL_READ_WRITEB)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field YCBCR422_INPUT[29:29] (RW)
 *
...
#define BP_ELCDIF_CTRL_YCBCR422_INPUT      (29)
#define BM_ELCDIF_CTRL_YCBCR422_INPUT      (0x20000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_YCBCR422_INPUT(v)   ((((reg32_t) v) << 29) & BM_ELCDIF_CTRL_YCBCR422_INPUT)
#else
#define BF_ELCDIF_CTRL_YCBCR422_INPUT(v)   (((v) << 29) & BM_ELCDIF_CTRL_YCBCR422_INPUT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field CLKGATE[30:30] (RW)
 *
 * This bit must be set to zero for normal operation. When set to one it gates off the clocks to the
 * block.
 */

#define BP_ELCDIF_CTRL_CLKGATE      (30)
#define BM_ELCDIF_CTRL_CLKGATE      (0x40000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_CLKGATE(v)   ((((reg32_t) v) << 30) & BM_ELCDIF_CTRL_CLKGATE)
#else
#define BF_ELCDIF_CTRL_CLKGATE(v)   (((v) << 30) & BM_ELCDIF_CTRL_CLKGATE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL, field SFTRST[31:31] (RW)
 *
 * This bit must be set to zero to enable normal operation of the eLCDIF. When set to one, it forces
 * a block level reset.
 */

#define BP_ELCDIF_CTRL_SFTRST      (31)
#define BM_ELCDIF_CTRL_SFTRST      (0x80000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL_SFTRST(v)   ((((reg32_t) v) << 31) & BM_ELCDIF_CTRL_SFTRST)
#else
#define BF_ELCDIF_CTRL_SFTRST(v)   (((v) << 31) & BM_ELCDIF_CTRL_SFTRST)
#endif
#ifndef __LANGUAGE_ASM__
...
typedef union
{
    ...
    struct
    {
        unsigned RESET : 1; //!< Reset bit for the external LCD controller. This bit can be changed at any time. It CANNOT be reset by SFTRST.
        unsigned MODE86 : 1; //!< This bit is used to select between the 8080 and 6800 series of microprocessor modes. This bit should only be changed when RUN is 0.
        unsigned BUSY_ENABLE : 1; //!< This bit enables the use of the interface's busy signal input. This should be enabled for LCD controllers that implement a busy line (to stall the eLCDIF from sending more data until ready). Otherwise this bit should be cleared.
        unsigned RESERVED0 : 5; //!< Reserved bits. Write as 0.
        unsigned VSYNC_EDGE_IRQ : 1; //!< This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is cleared by software by writing a one to its SCT clear address. It is set whenever the leading VSYNC edge is detected in the VSYNC and DOTCLK modes. In the DVI mode, it is asserted every time the block enters a new field.
        unsigned CUR_FRAME_DONE_IRQ : 1; //!< This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is cleared by software by writing a one to its SCT clear address. It indicates that the hardware has completed transmitting the current frame and is in the vertical blanking period in the DOTCLK/DVI modes. In the MPU and VSYNC modes, this IRQ is asserted at the end of the data transfer indicated by LCDIF_TRANSFER_COUNT register.
        unsigned UNDERFLOW_IRQ : 1; //!< This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is cleared by software by writing a one to its SCT clear address. A TXFIFO underflow in the write mode (MPU/VSYNC/DOTCLK/DVI mode) was detected. Could produce an error in the DOTCLK / DVI modes.
        unsigned OVERFLOW_IRQ : 1; //!< This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is cleared by software by writing a one to its SCT clear address. A latency FIFO (LFIFO) overflow in the write mode (MPU/VSYNC/DOTCLK/DVI mode) was detected, data samples have been lost.
        unsigned VSYNC_EDGE_IRQ_EN : 1; //!< This bit is set to enable an interrupt every time the hardware encounters the leading VSYNC edge in the VSYNC and DOTCLK modes, or the beginning of every field in DVI mode.
        unsigned CUR_FRAME_DONE_IRQ_EN : 1; //!< This bit is set to 1 enable an interrupt every time the hardware enters in the vertical blanking state.
        unsigned UNDERFLOW_IRQ_EN : 1; //!< This bit is set to enable an underflow interrupt in the TXFIFO in the write mode.
        unsigned OVERFLOW_IRQ_EN : 1; //!< This bit is set to enable an overflow interrupt in the TXFIFO in the write mode.
        unsigned BYTE_PACKING_FORMAT : 4; //!< This bitfield is used to show which data bytes in a 32-bit word are valid. Default value 0xf indicates that all bytes are valid. For 8-bit transfers, any combination in this bitfield will mean valid data is present in the corresponding bytes. In the 16-bit mode, a 16-bit half-word is valid only if adjacent bits [1:0] or [3:2] or both are 1. A value of 0x0 will mean that none of the bytes are valid and should not be used. For example, set the bit field value to 0x7 if the display data is arranged in the 24-bit unpacked format (A-R-G-B where A value does not have be transmitted). When input data is in YCbCr 4:2:2 format (YCBCR422_INPUT is 1), H_COUNT should be the number of pixels that should be fetched by the block and the BYTE_PACKING_FORMAT should be 0xF.(Note - YCBCR422_INPUT = 1 implies 2 pixels per 32 bits).
        unsigned IRQ_ON_ALTERNATE_FIELDS : 1; //!< If this bit is set, the eLCDIF block will assert the cur_frame_done interrupt only on alternate fields, otherwise it will issue the interrupt on both odd and even field. This bit is mostly relevant if INTERLACE_FIELDS is set. This feature is only available in DOTCLK and DVI modes.
        unsigned FIFO_CLEAR : 1; //!< Set this bit to clear all the data in the latency FIFO (LFIFO), TXFIFO and the RXFIFO.
        unsigned START_INTERLACE_FROM_SECOND_FIELD : 1; //!< The default is to grab the odd lines first and then the even lines. Set this bit if it is required to grab the even lines first and then the odd lines. (Line numbers start from 1, so odd lines are 1,3,5,etc. and even lines are 2,4,6, etc.)
        unsigned INTERLACE_FIELDS : 1; //!< Set this bit if it is required that the eLCDIF block fetches odd lines in one field and even lines in the other field. It will work only in LCDIF_MASTER is set to 1.
        unsigned RECOVER_ON_UNDERFLOW : 1; //!< Set this bit to enable the eLCDIF block to recover in the next field/frame if there was an underflow in the current field/frame.
        unsigned BM_ERROR_IRQ : 1; //!< This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is cleared by software by writing a one to its SCT clear address. This bit will be set when the eLCDIF is in master mode and an error response was returned by the slave.
        unsigned BM_ERROR_IRQ_EN : 1; //!< This bit is set to enable bus master error interrupt in the eLCDIF master mode.
        unsigned COMBINE_MPU_WR_STRB : 1; //!< If this bit is not set, the write strobe will be driven on LCD_WR_RWn pin in the 8080 mode and on the LCD_RD_E pin in the 6800 mode. If it is set, the write strobe of both the 6800 and 8080 modes will be driven only on the LCD_WR_RWn pin. Note that this does not work for read strobe.
        unsigned RESERVED1 : 4; //!< Reserved bits. Write as 0.
    } B;
} hw_elcdif_ctrl1_t;
#endif
...
#define HW_GPMI_CTRL0_SET(v)    ((*(volatile reg32_t *) HW_ELCDIF_CTRL1_SET_ADDR) = (v))
...
/* --- Register HW_ELCDIF_CTRL1, field RESET[0:0] (RW)
 *
...
#define BP_ELCDIF_CTRL1_RESET      (0)
#define BM_ELCDIF_CTRL1_RESET      (0x00000001)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_RESET(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CTRL1_RESET)
#else
#define BF_ELCDIF_CTRL1_RESET(v)   (((v) << 0) & BM_ELCDIF_CTRL1_RESET)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field MODE86[1:1] (RW)
 *
...
#define BP_ELCDIF_CTRL1_MODE86      (1)
#define BM_ELCDIF_CTRL1_MODE86      (0x00000002)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_MODE86(v)   ((((reg32_t) v) << 1) & BM_ELCDIF_CTRL1_MODE86)
#else
#define BF_ELCDIF_CTRL1_MODE86(v)   (((v) << 1) & BM_ELCDIF_CTRL1_MODE86)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field BUSY_ENABLE[2:2] (RW)
 *
...
#define BP_ELCDIF_CTRL1_BUSY_ENABLE      (2)
#define BM_ELCDIF_CTRL1_BUSY_ENABLE      (0x00000004)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_BUSY_ENABLE(v)   ((((reg32_t) v) << 2) & BM_ELCDIF_CTRL1_BUSY_ENABLE)
#else
#define BF_ELCDIF_CTRL1_BUSY_ENABLE(v)   (((v) << 2) & BM_ELCDIF_CTRL1_BUSY_ENABLE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field VSYNC_EDGE_IRQ[8:8] (RW)
 *
...
#define BP_ELCDIF_CTRL1_VSYNC_EDGE_IRQ      (8)
#define BM_ELCDIF_CTRL1_VSYNC_EDGE_IRQ      (0x00000100)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_VSYNC_EDGE_IRQ(v)   ((((reg32_t) v) << 8) & BM_ELCDIF_CTRL1_VSYNC_EDGE_IRQ)
#else
#define BF_ELCDIF_CTRL1_VSYNC_EDGE_IRQ(v)   (((v) << 8) & BM_ELCDIF_CTRL1_VSYNC_EDGE_IRQ)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field CUR_FRAME_DONE_IRQ[9:9] (RW)
 *
 * This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is
...
#define BP_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ      (9)
#define BM_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ      (0x00000200)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ(v)   ((((reg32_t) v) << 9) & BM_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ)
#else
#define BF_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ(v)   (((v) << 9) & BM_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field UNDERFLOW_IRQ[10:10] (RW)
 *
 * This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is
...
#define BP_ELCDIF_CTRL1_UNDERFLOW_IRQ      (10)
#define BM_ELCDIF_CTRL1_UNDERFLOW_IRQ      (0x00000400)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_UNDERFLOW_IRQ(v)   ((((reg32_t) v) << 10) & BM_ELCDIF_CTRL1_UNDERFLOW_IRQ)
#else
#define BF_ELCDIF_CTRL1_UNDERFLOW_IRQ(v)   (((v) << 10) & BM_ELCDIF_CTRL1_UNDERFLOW_IRQ)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field OVERFLOW_IRQ[11:11] (RW)
 *
 * This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is
...
#define BP_ELCDIF_CTRL1_OVERFLOW_IRQ      (11)
#define BM_ELCDIF_CTRL1_OVERFLOW_IRQ      (0x00000800)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_OVERFLOW_IRQ(v)   ((((reg32_t) v) << 11) & BM_ELCDIF_CTRL1_OVERFLOW_IRQ)
#else
#define BF_ELCDIF_CTRL1_OVERFLOW_IRQ(v)   (((v) << 11) & BM_ELCDIF_CTRL1_OVERFLOW_IRQ)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field VSYNC_EDGE_IRQ_EN[12:12] (RW)
 *
...
#define BP_ELCDIF_CTRL1_VSYNC_EDGE_IRQ_EN      (12)
#define BM_ELCDIF_CTRL1_VSYNC_EDGE_IRQ_EN      (0x00001000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_VSYNC_EDGE_IRQ_EN(v)   ((((reg32_t) v) << 12) & BM_ELCDIF_CTRL1_VSYNC_EDGE_IRQ_EN)
#else
#define BF_ELCDIF_CTRL1_VSYNC_EDGE_IRQ_EN(v)   (((v) << 12) & BM_ELCDIF_CTRL1_VSYNC_EDGE_IRQ_EN)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field CUR_FRAME_DONE_IRQ_EN[13:13] (RW)
 *
 * This bit is set to 1 enable an interrupt every time the hardware enters in the vertical blanking
 * state.
 */

#define BP_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ_EN      (13)
#define BM_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ_EN      (0x00002000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ_EN(v)   ((((reg32_t) v) << 13) & BM_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ_EN)
#else
#define BF_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ_EN(v)   (((v) << 13) & BM_ELCDIF_CTRL1_CUR_FRAME_DONE_IRQ_EN)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field UNDERFLOW_IRQ_EN[14:14] (RW)
 *
 * This bit is set to enable an underflow interrupt in the TXFIFO in the write mode.
 */

#define BP_ELCDIF_CTRL1_UNDERFLOW_IRQ_EN      (14)
#define BM_ELCDIF_CTRL1_UNDERFLOW_IRQ_EN      (0x00004000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_UNDERFLOW_IRQ_EN(v)   ((((reg32_t) v) << 14) & BM_ELCDIF_CTRL1_UNDERFLOW_IRQ_EN)
#else
#define BF_ELCDIF_CTRL1_UNDERFLOW_IRQ_EN(v)   (((v) << 14) & BM_ELCDIF_CTRL1_UNDERFLOW_IRQ_EN)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field OVERFLOW_IRQ_EN[15:15] (RW)
 *
 * This bit is set to enable an overflow interrupt in the TXFIFO in the write mode.
 */

#define BP_ELCDIF_CTRL1_OVERFLOW_IRQ_EN      (15)
#define BM_ELCDIF_CTRL1_OVERFLOW_IRQ_EN      (0x00008000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_OVERFLOW_IRQ_EN(v)   ((((reg32_t) v) << 15) & BM_ELCDIF_CTRL1_OVERFLOW_IRQ_EN)
#else
#define BF_ELCDIF_CTRL1_OVERFLOW_IRQ_EN(v)   (((v) << 15) & BM_ELCDIF_CTRL1_OVERFLOW_IRQ_EN)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_BYTE_PACKING_FORMAT(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CTRL1_BYTE_PACKING_FORMAT)
#else
#define BF_ELCDIF_CTRL1_BYTE_PACKING_FORMAT(v)   (((v) << 16) & BM_ELCDIF_CTRL1_BYTE_PACKING_FORMAT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field IRQ_ON_ALTERNATE_FIELDS[20:20] (RW)
 *
...
#define BP_ELCDIF_CTRL1_IRQ_ON_ALTERNATE_FIELDS      (20)
#define BM_ELCDIF_CTRL1_IRQ_ON_ALTERNATE_FIELDS      (0x00100000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_IRQ_ON_ALTERNATE_FIELDS(v)   ((((reg32_t) v) << 20) & BM_ELCDIF_CTRL1_IRQ_ON_ALTERNATE_FIELDS)
#else
#define BF_ELCDIF_CTRL1_IRQ_ON_ALTERNATE_FIELDS(v)   (((v) << 20) & BM_ELCDIF_CTRL1_IRQ_ON_ALTERNATE_FIELDS)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field FIFO_CLEAR[21:21] (RW)
 *
 * Set this bit to clear all the data in the latency FIFO (LFIFO), TXFIFO and the RXFIFO.
 */

#define BP_ELCDIF_CTRL1_FIFO_CLEAR      (21)
#define BM_ELCDIF_CTRL1_FIFO_CLEAR      (0x00200000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_FIFO_CLEAR(v)   ((((reg32_t) v) << 21) & BM_ELCDIF_CTRL1_FIFO_CLEAR)
#else
#define BF_ELCDIF_CTRL1_FIFO_CLEAR(v)   (((v) << 21) & BM_ELCDIF_CTRL1_FIFO_CLEAR)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field START_INTERLACE_FROM_SECOND_FIELD[22:22] (RW)
 *
...
#define BP_ELCDIF_CTRL1_START_INTERLACE_FROM_SECOND_FIELD      (22)
#define BM_ELCDIF_CTRL1_START_INTERLACE_FROM_SECOND_FIELD      (0x00400000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_START_INTERLACE_FROM_SECOND_FIELD(v)   ((((reg32_t) v) << 22) & BM_ELCDIF_CTRL1_START_INTERLACE_FROM_SECOND_FIELD)
#else
#define BF_ELCDIF_CTRL1_START_INTERLACE_FROM_SECOND_FIELD(v)   (((v) << 22) & BM_ELCDIF_CTRL1_START_INTERLACE_FROM_SECOND_FIELD)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field INTERLACE_FIELDS[23:23] (RW)
 *
...
#define BP_ELCDIF_CTRL1_INTERLACE_FIELDS      (23)
#define BM_ELCDIF_CTRL1_INTERLACE_FIELDS      (0x00800000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_INTERLACE_FIELDS(v)   ((((reg32_t) v) << 23) & BM_ELCDIF_CTRL1_INTERLACE_FIELDS)
#else
#define BF_ELCDIF_CTRL1_INTERLACE_FIELDS(v)   (((v) << 23) & BM_ELCDIF_CTRL1_INTERLACE_FIELDS)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field RECOVER_ON_UNDERFLOW[24:24] (RW)
 *
...
#define BP_ELCDIF_CTRL1_RECOVER_ON_UNDERFLOW      (24)
#define BM_ELCDIF_CTRL1_RECOVER_ON_UNDERFLOW      (0x01000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_RECOVER_ON_UNDERFLOW(v)   ((((reg32_t) v) << 24) & BM_ELCDIF_CTRL1_RECOVER_ON_UNDERFLOW)
#else
#define BF_ELCDIF_CTRL1_RECOVER_ON_UNDERFLOW(v)   (((v) << 24) & BM_ELCDIF_CTRL1_RECOVER_ON_UNDERFLOW)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field BM_ERROR_IRQ[25:25] (RW)
 *
 * This bit is set to indicate that an interrupt is requested by the eLCDIF block. This bit is
...
#define BP_ELCDIF_CTRL1_BM_ERROR_IRQ      (25)
#define BM_ELCDIF_CTRL1_BM_ERROR_IRQ      (0x02000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_BM_ERROR_IRQ(v)   ((((reg32_t) v) << 25) & BM_ELCDIF_CTRL1_BM_ERROR_IRQ)
#else
#define BF_ELCDIF_CTRL1_BM_ERROR_IRQ(v)   (((v) << 25) & BM_ELCDIF_CTRL1_BM_ERROR_IRQ)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field BM_ERROR_IRQ_EN[26:26] (RW)
 *
 * This bit is set to enable bus master error interrupt in the eLCDIF master mode.
 */

#define BP_ELCDIF_CTRL1_BM_ERROR_IRQ_EN      (26)
#define BM_ELCDIF_CTRL1_BM_ERROR_IRQ_EN      (0x04000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_BM_ERROR_IRQ_EN(v)   ((((reg32_t) v) << 26) & BM_ELCDIF_CTRL1_BM_ERROR_IRQ_EN)
#else
#define BF_ELCDIF_CTRL1_BM_ERROR_IRQ_EN(v)   (((v) << 26) & BM_ELCDIF_CTRL1_BM_ERROR_IRQ_EN)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL1, field COMBINE_MPU_WR_STRB[27:27] (RW)
 *
...
#define BP_ELCDIF_CTRL1_COMBINE_MPU_WR_STRB      (27)
#define BM_ELCDIF_CTRL1_COMBINE_MPU_WR_STRB      (0x08000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL1_COMBINE_MPU_WR_STRB(v)   ((((reg32_t) v) << 27) & BM_ELCDIF_CTRL1_COMBINE_MPU_WR_STRB)
#else
#define BF_ELCDIF_CTRL1_COMBINE_MPU_WR_STRB(v)   (((v) << 27) & BM_ELCDIF_CTRL1_COMBINE_MPU_WR_STRB)
#endif
#ifndef __LANGUAGE_ASM__
...
typedef union
{
    ...
    {
        unsigned RESERVED0 : 1; //!< Reserved bits. Write as 0.
        unsigned INITIAL_DUMMY_READ : 3; //!< The value in this field determines the number of dummy 8/16/18/24-bit subwords that have to be read back from the LCD panel/controller. They will then not be stored in the read FIFO.
        unsigned READ_MODE_NUM_PACKED_SUBWORDS : 3; //!< Indicates the number of valid 8/16/18/24-bit subwords that will be packed into the 32-bit word in read mode. The subword size (8,16, 18 or 24 bits) is determined by the LCD_DATABUS_WIDTH field. The swizzle operation is performed after READ_MODE_NUM_PACKED_SUBWORDS number of subwords has been received and stored in little-endian format. For example, if LCD_DATABUS_WIDTH is set to 8-bit and data to be read back has to be stored in memory in 24-bit unpacked RGB format, set READ_MODE_NUM_PACKED_SUBWORDS to 0x3 so that each 32-bit word will contain only 3 valid bytes (RGB). Maximum value of READ_MODE_NUM_PACKED_SUBWORDS is 4 for 8-bit databus, 2 for 16-bit databus and 1 for 18/24-bit databus.
        unsigned RESERVED1 : 1; //!< Reserved bits. Write as 0.
        unsigned READ_MODE_6_BIT_INPUT : 1; //!< Setting this bit to 1 indicates to eLCDIF that even though LCD_DATABUS_WIDTH is set to 8 bits, the input data is actually only 6 bits wide and exists on D5-D0.
        unsigned READ_MODE_OUTPUT_IN_RGB_FORMAT : 1; //!< Setting this bit will enable the eLCDIF to convert the incoming data to the RGB format given by WORD_LENGTH bitfield. This feature is not available when WORD_LENGTH is set to 8 bits. eLCDIF performs this operation of converting to RGB format after the endianness has been determined by the READ_PACK_DIR bitfield.
        unsigned READ_PACK_DIR : 1; //!< The default value of 0 indicates data is stored in the little endian format. When LCD_DATABUS_WIDTH is 8-bit, this bit provides the option of rearranging the data byte-wise in the big endian format. For example, if READ_MODE_NUM_PACKED_SUBWORDS = 3 and the order of incoming data is 0x11, 0x22 and 0x33, then setting this bit to 1 will cause the data to be stored as 0x00112233 as opposed to the default 0x00332211. This operation occurs after the shifting operation done by SHIFT_NUM_BITS bitfield.
        unsigned RESERVED2 : 1; //!< Reserved bits. Write as 0.
        unsigned EVEN_LINE_PATTERN : 3; //!< This field determines the order of the RGB components of each pixel in EVEN lines (line numbers 2,4,6,..). This bitfield must be 0 in DVI mode.
        unsigned RESERVED3 : 1; //!< Reserved bits. Write as 0.
        unsigned ODD_LINE_PATTERN : 3; //!< This field determines the order of the RGB components of each pixel in ODD lines (line numbers 1,3,5,..). This bitfield must be 0 in DVI mode.
        unsigned RESERVED4 : 1; //!< Reserved bits. Write as 0.
        unsigned BURST_LEN_8 : 1; //!< By default, when the eLCDIF is in the bus master mode, it will issue AXI bursts of length 16 (except when in packed 24 bpp mode, it will issue bursts of length 15). When this bit is set to 1, the block will issue bursts of length 8 (except when in packed 24 bpp mode, it will issue bursts of length 9). Note that this bitfield is only applicable when LCDIF_MASTER is set to 1.
        unsigned OUTSTANDING_REQS : 3; //!< This bitfield indicates the maximum number of outstanding transactions that eLCDIF should request when it is acting as a bus master. Default is 2 outstanding transactions.
        unsigned RESERVED5 : 8; //!< Reserved bits. Write as 0.
    } B;
} hw_elcdif_ctrl2_t;
#endif
...
#define HW_GPMI_CTRL0_SET(v)    ((*(volatile reg32_t *) HW_ELCDIF_CTRL2_SET_ADDR) = (v))
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_INITIAL_DUMMY_READ(v)   ((((reg32_t) v) << 1) & BM_ELCDIF_CTRL2_INITIAL_DUMMY_READ)
#else
#define BF_ELCDIF_CTRL2_INITIAL_DUMMY_READ(v)   (((v) << 1) & BM_ELCDIF_CTRL2_INITIAL_DUMMY_READ)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_READ_MODE_NUM_PACKED_SUBWORDS(v)   ((((reg32_t) v) << 4) & BM_ELCDIF_CTRL2_READ_MODE_NUM_PACKED_SUBWORDS)
#else
#define BF_ELCDIF_CTRL2_READ_MODE_NUM_PACKED_SUBWORDS(v)   (((v) << 4) & BM_ELCDIF_CTRL2_READ_MODE_NUM_PACKED_SUBWORDS)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL2, field READ_MODE_6_BIT_INPUT[8:8] (RW)
 *
...
#define BP_ELCDIF_CTRL2_READ_MODE_6_BIT_INPUT      (8)
#define BM_ELCDIF_CTRL2_READ_MODE_6_BIT_INPUT      (0x00000100)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_READ_MODE_6_BIT_INPUT(v)   ((((reg32_t) v) << 8) & BM_ELCDIF_CTRL2_READ_MODE_6_BIT_INPUT)
#else
#define BF_ELCDIF_CTRL2_READ_MODE_6_BIT_INPUT(v)   (((v) << 8) & BM_ELCDIF_CTRL2_READ_MODE_6_BIT_INPUT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL2, field READ_MODE_OUTPUT_IN_RGB_FORMAT[9:9] (RW)
 *
...
#define BP_ELCDIF_CTRL2_READ_MODE_OUTPUT_IN_RGB_FORMAT      (9)
#define BM_ELCDIF_CTRL2_READ_MODE_OUTPUT_IN_RGB_FORMAT      (0x00000200)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_READ_MODE_OUTPUT_IN_RGB_FORMAT(v)   ((((reg32_t) v) << 9) & BM_ELCDIF_CTRL2_READ_MODE_OUTPUT_IN_RGB_FORMAT)
#else
#define BF_ELCDIF_CTRL2_READ_MODE_OUTPUT_IN_RGB_FORMAT(v)   (((v) << 9) & BM_ELCDIF_CTRL2_READ_MODE_OUTPUT_IN_RGB_FORMAT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL2, field READ_PACK_DIR[10:10] (RW)
 *
...
#define BP_ELCDIF_CTRL2_READ_PACK_DIR      (10)
#define BM_ELCDIF_CTRL2_READ_PACK_DIR      (0x00000400)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_READ_PACK_DIR(v)   ((((reg32_t) v) << 10) & BM_ELCDIF_CTRL2_READ_PACK_DIR)
#else
#define BF_ELCDIF_CTRL2_READ_PACK_DIR(v)   (((v) << 10) & BM_ELCDIF_CTRL2_READ_PACK_DIR)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_EVEN_LINE_PATTERN(v)   ((((reg32_t) v) << 12) & BM_ELCDIF_CTRL2_EVEN_LINE_PATTERN)
#else
#define BF_ELCDIF_CTRL2_EVEN_LINE_PATTERN(v)   (((v) << 12) & BM_ELCDIF_CTRL2_EVEN_LINE_PATTERN)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_ODD_LINE_PATTERN(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CTRL2_ODD_LINE_PATTERN)
#else
#define BF_ELCDIF_CTRL2_ODD_LINE_PATTERN(v)   (((v) << 16) & BM_ELCDIF_CTRL2_ODD_LINE_PATTERN)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_CTRL2, field BURST_LEN_8[20:20] (RW)
 *
...
#define BP_ELCDIF_CTRL2_BURST_LEN_8      (20)
#define BM_ELCDIF_CTRL2_BURST_LEN_8      (0x00100000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_BURST_LEN_8(v)   ((((reg32_t) v) << 20) & BM_ELCDIF_CTRL2_BURST_LEN_8)
#else
#define BF_ELCDIF_CTRL2_BURST_LEN_8(v)   (((v) << 20) & BM_ELCDIF_CTRL2_BURST_LEN_8)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CTRL2_OUTSTANDING_REQS(v)   ((((reg32_t) v) << 21) & BM_ELCDIF_CTRL2_OUTSTANDING_REQS)
#else
#define BF_ELCDIF_CTRL2_OUTSTANDING_REQS(v)   (((v) << 21) & BM_ELCDIF_CTRL2_OUTSTANDING_REQS)
#endif
#ifndef __LANGUAGE_ASM__
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_TRANSFER_COUNT_H_COUNT(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_TRANSFER_COUNT_H_COUNT)
#else
#define BF_ELCDIF_TRANSFER_COUNT_H_COUNT(v)   (((v) << 0) & BM_ELCDIF_TRANSFER_COUNT_H_COUNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the H_COUNT field to a new value.
#define BW_ELCDIF_TRANSFER_COUNT_H_COUNT(v)   BF_CS1(ELCDIF_TRANSFER_COUNT, H_COUNT, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_TRANSFER_COUNT_V_COUNT(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_TRANSFER_COUNT_V_COUNT)
#else
#define BF_ELCDIF_TRANSFER_COUNT_V_COUNT(v)   (((v) << 16) & BM_ELCDIF_TRANSFER_COUNT_V_COUNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the V_COUNT field to a new value.
#define BW_ELCDIF_TRANSFER_COUNT_V_COUNT(v)   BF_CS1(ELCDIF_TRANSFER_COUNT, V_COUNT, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CUR_BUF_ADDR(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CUR_BUF_ADDR)
#else
#define BF_ELCDIF_CUR_BUF_ADDR(v)   (((v) << 0) & BM_ELCDIF_CUR_BUF_ADDR)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the ADDR field to a new value.
#define BW_ELCDIF_CUR_BUF_ADDR(v)   BF_CS1(ELCDIF_CUR_BUF, ADDR, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_NEXT_BUF_ADDR(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_NEXT_BUF_ADDR)
#else
#define BF_ELCDIF_NEXT_BUF_ADDR(v)   (((v) << 0) & BM_ELCDIF_NEXT_BUF_ADDR)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the ADDR field to a new value.
#define BW_ELCDIF_NEXT_BUF_ADDR(v)   BF_CS1(ELCDIF_NEXT_BUF, ADDR, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned DATA_SETUP : 8; //!< Data bus setup time in CLK_DIS_LCDIFn cycles. Also the time that the data strobe is asserted in a cycle.
        unsigned DATA_HOLD : 8; //!< Data bus hold time in CLK_DIS_LCDIFn cycles. Also the time that the data strobe is de-asserted in a cycle
        unsigned CMD_SETUP : 8; //!< Number of CLK_DIS_LCDIFn cycles that the the DCn signal is active before CEn is asserted.
        unsigned CMD_HOLD : 8; //!< Number of CLK_DIS_LCDIFn cycles that the DCn signal is active after CEn is deasserted.
    } B;
} hw_elcdif_timing_t;
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_TIMING_DATA_SETUP(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_TIMING_DATA_SETUP)
#else
#define BF_ELCDIF_TIMING_DATA_SETUP(v)   (((v) << 0) & BM_ELCDIF_TIMING_DATA_SETUP)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the DATA_SETUP field to a new value.
#define BW_ELCDIF_TIMING_DATA_SETUP(v)   BF_CS1(ELCDIF_TIMING, DATA_SETUP, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_TIMING_DATA_HOLD(v)   ((((reg32_t) v) << 8) & BM_ELCDIF_TIMING_DATA_HOLD)
#else
#define BF_ELCDIF_TIMING_DATA_HOLD(v)   (((v) << 8) & BM_ELCDIF_TIMING_DATA_HOLD)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the DATA_HOLD field to a new value.
#define BW_ELCDIF_TIMING_DATA_HOLD(v)   BF_CS1(ELCDIF_TIMING, DATA_HOLD, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_TIMING_CMD_SETUP(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_TIMING_CMD_SETUP)
#else
#define BF_ELCDIF_TIMING_CMD_SETUP(v)   (((v) << 16) & BM_ELCDIF_TIMING_CMD_SETUP)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CMD_SETUP field to a new value.
#define BW_ELCDIF_TIMING_CMD_SETUP(v)   BF_CS1(ELCDIF_TIMING, CMD_SETUP, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_TIMING_CMD_HOLD(v)   ((((reg32_t) v) << 24) & BM_ELCDIF_TIMING_CMD_HOLD)
#else
#define BF_ELCDIF_TIMING_CMD_HOLD(v)   (((v) << 24) & BM_ELCDIF_TIMING_CMD_HOLD)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CMD_HOLD field to a new value.
#define BW_ELCDIF_TIMING_CMD_HOLD(v)   BF_CS1(ELCDIF_TIMING, CMD_HOLD, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned VSYNC_PULSE_WIDTH : 18; //!< Number of units for which VSYNC signal is active. For the DOTCLK mode, the unit is determined by the VSYNC_PULSE_WIDTH_UNIT. If the VSYNC_PULSE_WIDTH_UNIT is 0 for DOTCLK mode, VSYNC_PULSE_WIDTH must be less than HSYNC_PERIOD. For the VSYNC interface mode, it should be in terms of number of CLK_DIS_LCDIFn cycles only.
        unsigned HALF_LINE_MODE : 1; //!< When this bit is 0, the first field (VSYNC period) will end in half a horizontal line and the second field will begin with half a horizontal line. When this bit is 1, all fields will end with half a horizontal line, and none will begin with half a horizontal line.
        unsigned HALF_LINE : 1; //!< Setting this bit to 1 will make the total VSYNC period equal to the VSYNC_PERIOD field plus half the HORIZONTAL_PERIOD field (i.e. VSYNC_PERIOD field plus half horizontal line), otherwise it is just VSYNC_PERIOD. Should be only used in the DOTCLK mode, not in the VSYNC interface mode.
        unsigned VSYNC_PULSE_WIDTH_UNIT : 1; //!< Default 0 for counting VSYNC_PULSE_WIDTH in terms of CLK_DIS_LCDIFn cycles. Set it to 1 to count in terms of complete horizontal lines.
        unsigned VSYNC_PERIOD_UNIT : 1; //!< Default 0 for counting VSYNC_PERIOD in terms of CLK_DIS_LCDIFn cycles. Set it to 1 to count in terms of complete horizontal lines. CLK_DIS_LCDIFn cycles should be used in the VSYNC mode, while horizontal line should be used in the DOTCLK mode.
        unsigned RESERVED0 : 2; //!< Reserved bits. Write as 0.
        unsigned ENABLE_POL : 1; //!< Default 0 active low during valid data transfer on each horizontal line.
        unsigned DOTCLK_POL : 1; //!< Default is data launched at negative edge of DOTCLK and captured at positive edge. Set it to 1 to invert the polarity. Set it to 0 in DVI mode.
        unsigned HSYNC_POL : 1; //!< Default 0 active low during HSYNC_PULSE_WIDTH time and will be high during the rest of the HSYNC period. Set it to 1 to invert the polarity.
        unsigned VSYNC_POL : 1; //!< Default 0 active low during VSYNC_PULSE_WIDTH time and will be high during the rest of the VSYNC period. Set it to 1 to invert the polarity.
        unsigned ENABLE_PRESENT : 1; //!< Setting this bit to 1 will make the hardware generate the ENABLE signal in the DOTCLK mode, thereby making it the true RGB interface along with the remaining three signals VSYNC, HSYNC and DOTCLK.
        unsigned VSYNC_OEB : 1; //!< 0 means the VSYNC signal is an output, 1 means it is an input. Should be set to 0 in the DOTCLK mode.
        unsigned RESERVED1 : 2; //!< Reserved bits. Write as 0.
    } B;
} hw_elcdif_vdctrl0_t;
#endif
...
#define HW_GPMI_CTRL0_SET(v)    ((*(volatile reg32_t *) HW_ELCDIF_VDCTRL0_SET_ADDR) = (v))
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH)
#else
#define BF_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH(v)   (((v) << 0) & BM_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field HALF_LINE_MODE[18:18] (RW)
 *
...
#define BP_ELCDIF_VDCTRL0_HALF_LINE_MODE      (18)
#define BM_ELCDIF_VDCTRL0_HALF_LINE_MODE      (0x00040000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_HALF_LINE_MODE(v)   ((((reg32_t) v) << 18) & BM_ELCDIF_VDCTRL0_HALF_LINE_MODE)
#else
#define BF_ELCDIF_VDCTRL0_HALF_LINE_MODE(v)   (((v) << 18) & BM_ELCDIF_VDCTRL0_HALF_LINE_MODE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field HALF_LINE[19:19] (RW)
 *
...
#define BP_ELCDIF_VDCTRL0_HALF_LINE      (19)
#define BM_ELCDIF_VDCTRL0_HALF_LINE      (0x00080000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_HALF_LINE(v)   ((((reg32_t) v) << 19) & BM_ELCDIF_VDCTRL0_HALF_LINE)
#else
#define BF_ELCDIF_VDCTRL0_HALF_LINE(v)   (((v) << 19) & BM_ELCDIF_VDCTRL0_HALF_LINE)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field VSYNC_PULSE_WIDTH_UNIT[20:20] (RW)
 *
...
#define BP_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH_UNIT      (20)
#define BM_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH_UNIT      (0x00100000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH_UNIT(v)   ((((reg32_t) v) << 20) & BM_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH_UNIT)
#else
#define BF_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH_UNIT(v)   (((v) << 20) & BM_ELCDIF_VDCTRL0_VSYNC_PULSE_WIDTH_UNIT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field VSYNC_PERIOD_UNIT[21:21] (RW)
 *
...
#define BP_ELCDIF_VDCTRL0_VSYNC_PERIOD_UNIT      (21)
#define BM_ELCDIF_VDCTRL0_VSYNC_PERIOD_UNIT      (0x00200000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_VSYNC_PERIOD_UNIT(v)   ((((reg32_t) v) << 21) & BM_ELCDIF_VDCTRL0_VSYNC_PERIOD_UNIT)
#else
#define BF_ELCDIF_VDCTRL0_VSYNC_PERIOD_UNIT(v)   (((v) << 21) & BM_ELCDIF_VDCTRL0_VSYNC_PERIOD_UNIT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field ENABLE_POL[24:24] (RW)
 *
 * Default 0 active low during valid data transfer on each horizontal line.
 */

#define BP_ELCDIF_VDCTRL0_ENABLE_POL      (24)
#define BM_ELCDIF_VDCTRL0_ENABLE_POL      (0x01000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_ENABLE_POL(v)   ((((reg32_t) v) << 24) & BM_ELCDIF_VDCTRL0_ENABLE_POL)
#else
#define BF_ELCDIF_VDCTRL0_ENABLE_POL(v)   (((v) << 24) & BM_ELCDIF_VDCTRL0_ENABLE_POL)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field DOTCLK_POL[25:25] (RW)
 *
...
#define BP_ELCDIF_VDCTRL0_DOTCLK_POL      (25)
#define BM_ELCDIF_VDCTRL0_DOTCLK_POL      (0x02000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_DOTCLK_POL(v)   ((((reg32_t) v) << 25) & BM_ELCDIF_VDCTRL0_DOTCLK_POL)
#else
#define BF_ELCDIF_VDCTRL0_DOTCLK_POL(v)   (((v) << 25) & BM_ELCDIF_VDCTRL0_DOTCLK_POL)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field HSYNC_POL[26:26] (RW)
 *
...
#define BP_ELCDIF_VDCTRL0_HSYNC_POL      (26)
#define BM_ELCDIF_VDCTRL0_HSYNC_POL      (0x04000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_HSYNC_POL(v)   ((((reg32_t) v) << 26) & BM_ELCDIF_VDCTRL0_HSYNC_POL)
#else
#define BF_ELCDIF_VDCTRL0_HSYNC_POL(v)   (((v) << 26) & BM_ELCDIF_VDCTRL0_HSYNC_POL)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field VSYNC_POL[27:27] (RW)
 *
 * Default 0 active low during VSYNC_PULSE_WIDTH time and will be high during the rest of the VSYNC
 * period. Set it to 1 to invert the polarity.
 */

#define BP_ELCDIF_VDCTRL0_VSYNC_POL      (27)
#define BM_ELCDIF_VDCTRL0_VSYNC_POL      (0x08000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_VSYNC_POL(v)   ((((reg32_t) v) << 27) & BM_ELCDIF_VDCTRL0_VSYNC_POL)
#else
#define BF_ELCDIF_VDCTRL0_VSYNC_POL(v)   (((v) << 27) & BM_ELCDIF_VDCTRL0_VSYNC_POL)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field ENABLE_PRESENT[28:28] (RW)
 *
...
#define BP_ELCDIF_VDCTRL0_ENABLE_PRESENT      (28)
#define BM_ELCDIF_VDCTRL0_ENABLE_PRESENT      (0x10000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_ENABLE_PRESENT(v)   ((((reg32_t) v) << 28) & BM_ELCDIF_VDCTRL0_ENABLE_PRESENT)
#else
#define BF_ELCDIF_VDCTRL0_ENABLE_PRESENT(v)   (((v) << 28) & BM_ELCDIF_VDCTRL0_ENABLE_PRESENT)
#endif
#ifndef __LANGUAGE_ASM__
...
/* --- Register HW_ELCDIF_VDCTRL0, field VSYNC_OEB[29:29] (RW)
 *
...
#define BP_ELCDIF_VDCTRL0_VSYNC_OEB      (29)
#define BM_ELCDIF_VDCTRL0_VSYNC_OEB      (0x20000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL0_VSYNC_OEB(v)   ((((reg32_t) v) << 29) & BM_ELCDIF_VDCTRL0_VSYNC_OEB)
#else
#define BF_ELCDIF_VDCTRL0_VSYNC_OEB(v)   (((v) << 29) & BM_ELCDIF_VDCTRL0_VSYNC_OEB)
#endif
#ifndef __LANGUAGE_ASM__
...
typedef union
{
    ...
    struct
    {
        unsigned VSYNC_PERIOD : 32; //!< Total number of units between two positive or two negative edges of the VSYNC signal. If HALF_LINE is set, it is implicitly calculated to be VSYNC_PERIOD plus half HSYNC_PERIOD.
    } B;
} hw_elcdif_vdctrl1_t;
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL1_VSYNC_PERIOD(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_VDCTRL1_VSYNC_PERIOD)
#else
#define BF_ELCDIF_VDCTRL1_VSYNC_PERIOD(v)   (((v) << 0) & BM_ELCDIF_VDCTRL1_VSYNC_PERIOD)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the VSYNC_PERIOD field to a new value.
#define BW_ELCDIF_VDCTRL1_VSYNC_PERIOD(v)   BF_CS1(ELCDIF_VDCTRL1, VSYNC_PERIOD, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned HSYNC_PERIOD : 18; //!< Total number of CLK_DIS_LCDIFn cycles between two positive or two negative edges of the HSYNC signal.
        unsigned HSYNC_PULSE_WIDTH : 14; //!< Number of CLK_DIS_LCDIFn cycles for which HSYNC signal is active.
    } B;
} hw_elcdif_vdctrl2_t;
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL2_HSYNC_PERIOD(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_VDCTRL2_HSYNC_PERIOD)
#else
#define BF_ELCDIF_VDCTRL2_HSYNC_PERIOD(v)   (((v) << 0) & BM_ELCDIF_VDCTRL2_HSYNC_PERIOD)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the HSYNC_PERIOD field to a new value.
#define BW_ELCDIF_VDCTRL2_HSYNC_PERIOD(v)   BF_CS1(ELCDIF_VDCTRL2, HSYNC_PERIOD, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL2_HSYNC_PULSE_WIDTH(v)   ((((reg32_t) v) << 18) & BM_ELCDIF_VDCTRL2_HSYNC_PULSE_WIDTH)
#else
#define BF_ELCDIF_VDCTRL2_HSYNC_PULSE_WIDTH(v)   (((v) << 18) & BM_ELCDIF_VDCTRL2_HSYNC_PULSE_WIDTH)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the HSYNC_PULSE_WIDTH field to a new value.
#define BW_ELCDIF_VDCTRL2_HSYNC_PULSE_WIDTH(v)   BF_CS1(ELCDIF_VDCTRL2, HSYNC_PULSE_WIDTH, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned VERTICAL_WAIT_CNT : 16; //!< In the VSYNC interface mode, wait for this number of CLK_DIS_LCDIFn cycles from the falling VSYNC edge (or rising if VSYNC_POL is 1) before starting LCD transactions and is applicable only if WAIT_FOR_VSYNC_EDGE is set. Minimum is CMD_SETUP+5. In the DOTCLK mode, it accounts for the veritcal back porch lines plus the number of horizontal lines before the moving picture begins. The unit for this parameter is inherently the same as the VSYNC_PERIOD_UNIT.
        unsigned HORIZONTAL_WAIT_CNT : 12; //!< In the DOTCLK mode, wait for this number of clocks from falling edge (or rising if HSYNC_POL is 1) of HSYNC signal to account for horizontal back porch plus the number of DOTCLKs before the moving picture information begins.
        unsigned VSYNC_ONLY : 1; //!< This bit must be set to 1 in the VSYNC mode of operation, and 0 in the DOTCLK mode of operation.
        unsigned MUX_SYNC_SIGNALS : 1; //!< When this bit is set, the eLCDIF block will internally mux HSYNC with LCD_D14, DOTCLK with LCD_D13 and ENABLE with LCD_D12, otherwise these signals will go out on separate pins. This feature can be used to maintain backward compatability with 37xx.
        unsigned RESERVED0 : 2; //!< Reserved bits, write as 0.
    } B;
} hw_elcdif_vdctrl3_t;
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL3_VERTICAL_WAIT_CNT(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_VDCTRL3_VERTICAL_WAIT_CNT)
#else
#define BF_ELCDIF_VDCTRL3_VERTICAL_WAIT_CNT(v)   (((v) << 0) & BM_ELCDIF_VDCTRL3_VERTICAL_WAIT_CNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the VERTICAL_WAIT_CNT field to a new value.
#define BW_ELCDIF_VDCTRL3_VERTICAL_WAIT_CNT(v)   BF_CS1(ELCDIF_VDCTRL3, VERTICAL_WAIT_CNT, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL3_HORIZONTAL_WAIT_CNT(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_VDCTRL3_HORIZONTAL_WAIT_CNT)
#else
#define BF_ELCDIF_VDCTRL3_HORIZONTAL_WAIT_CNT(v)   (((v) << 16) & BM_ELCDIF_VDCTRL3_HORIZONTAL_WAIT_CNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the HORIZONTAL_WAIT_CNT field to a new value.
#define BW_ELCDIF_VDCTRL3_HORIZONTAL_WAIT_CNT(v)   BF_CS1(ELCDIF_VDCTRL3, HORIZONTAL_WAIT_CNT, v)
#endif

/* --- Register HW_ELCDIF_VDCTRL3, field VSYNC_ONLY[28:28] (RW)
 *
 * This bit must be set to 1 in the VSYNC mode of operation, and 0 in the DOTCLK mode of operation.
 */

#define BP_ELCDIF_VDCTRL3_VSYNC_ONLY      (28)
#define BM_ELCDIF_VDCTRL3_VSYNC_ONLY      (0x10000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL3_VSYNC_ONLY(v)   ((((reg32_t) v) << 28) & BM_ELCDIF_VDCTRL3_VSYNC_ONLY)
#else
#define BF_ELCDIF_VDCTRL3_VSYNC_ONLY(v)   (((v) << 28) & BM_ELCDIF_VDCTRL3_VSYNC_ONLY)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the VSYNC_ONLY field to a new value.
#define BW_ELCDIF_VDCTRL3_VSYNC_ONLY(v)   BF_CS1(ELCDIF_VDCTRL3, VSYNC_ONLY, v)
#endif

/* --- Register HW_ELCDIF_VDCTRL3, field MUX_SYNC_SIGNALS[29:29] (RW)
 *
...
#define BP_ELCDIF_VDCTRL3_MUX_SYNC_SIGNALS      (29)
#define BM_ELCDIF_VDCTRL3_MUX_SYNC_SIGNALS      (0x20000000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL3_MUX_SYNC_SIGNALS(v)   ((((reg32_t) v) << 29) & BM_ELCDIF_VDCTRL3_MUX_SYNC_SIGNALS)
#else
#define BF_ELCDIF_VDCTRL3_MUX_SYNC_SIGNALS(v)   (((v) << 29) & BM_ELCDIF_VDCTRL3_MUX_SYNC_SIGNALS)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the MUX_SYNC_SIGNALS field to a new value.
#define BW_ELCDIF_VDCTRL3_MUX_SYNC_SIGNALS(v)   BF_CS1(ELCDIF_VDCTRL3, MUX_SYNC_SIGNALS, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned DOTCLK_H_VALID_DATA_CNT : 18; //!< Total number of CLK_DIS_LCDIFn cycles on each horizontal line that carry valid data in DOTCLK mode.
        unsigned SYNC_SIGNALS_ON : 1; //!< Set this field to 1 if the LCD controller requires that the VSYNC or VSYNC/HSYNC/DOTCLK control signals should be active atleast one frame before the data transfers actually start and remain active atleast one frame after the data transfers end. The hardware does not count the number of frames automatically. Rather, the VSYNC edge interrupt can be monitored by software to count the number of frames that have occured after this bit is set and then the RUN bit can be set to start the data transactions. This bit must always be set in the DOTCLK mode of operation, and it must be set in the VSYNC mode of operation when VSYNC signal is an output.
        unsigned RESERVED0 : 10; //!< Reserved bits, write as 0.
        unsigned DOTCLK_DLY_SEL : 3; //!< This bitfield selects the amount of time by which the DOTCLK signal should be delayed before coming out of the LCD_DOTCK pin. 0 = 2ns; 1=4ns;2=6ns;3=8ns. Remaining values are reserved.
    } B;
} hw_elcdif_vdctrl4_t;
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL4_DOTCLK_H_VALID_DATA_CNT(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_VDCTRL4_DOTCLK_H_VALID_DATA_CNT)
#else
#define BF_ELCDIF_VDCTRL4_DOTCLK_H_VALID_DATA_CNT(v)   (((v) << 0) & BM_ELCDIF_VDCTRL4_DOTCLK_H_VALID_DATA_CNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the DOTCLK_H_VALID_DATA_CNT field to a new value.
#define BW_ELCDIF_VDCTRL4_DOTCLK_H_VALID_DATA_CNT(v)   BF_CS1(ELCDIF_VDCTRL4, DOTCLK_H_VALID_DATA_CNT, v)
#endif

/* --- Register HW_ELCDIF_VDCTRL4, field SYNC_SIGNALS_ON[18:18] (RW)
 *
...
#define BP_ELCDIF_VDCTRL4_SYNC_SIGNALS_ON      (18)
#define BM_ELCDIF_VDCTRL4_SYNC_SIGNALS_ON      (0x00040000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL4_SYNC_SIGNALS_ON(v)   ((((reg32_t) v) << 18) & BM_ELCDIF_VDCTRL4_SYNC_SIGNALS_ON)
#else
#define BF_ELCDIF_VDCTRL4_SYNC_SIGNALS_ON(v)   (((v) << 18) & BM_ELCDIF_VDCTRL4_SYNC_SIGNALS_ON)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the SYNC_SIGNALS_ON field to a new value.
#define BW_ELCDIF_VDCTRL4_SYNC_SIGNALS_ON(v)   BF_CS1(ELCDIF_VDCTRL4, SYNC_SIGNALS_ON, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_VDCTRL4_DOTCLK_DLY_SEL(v)   ((((reg32_t) v) << 29) & BM_ELCDIF_VDCTRL4_DOTCLK_DLY_SEL)
#else
#define BF_ELCDIF_VDCTRL4_DOTCLK_DLY_SEL(v)   (((v) << 29) & BM_ELCDIF_VDCTRL4_DOTCLK_DLY_SEL)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the DOTCLK_DLY_SEL field to a new value.
#define BW_ELCDIF_VDCTRL4_DOTCLK_DLY_SEL(v)   BF_CS1(ELCDIF_VDCTRL4, DOTCLK_DLY_SEL, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned H_BLANKING_CNT : 12; //!< Number of blanking samples to be inserted between EAV and SAV during horizontal blanking interval.
        unsigned RESERVED0 : 4; //!< Reserved bits, write as 0.
        unsigned H_ACTIVE_CNT : 12; //!< Number of active video samples to be transmitted. (Mostly will be 1440 for both PAL and NTSC). Must always be a multiple of 4.
        unsigned RESERVED1 : 4; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL0_H_BLANKING_CNT(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_DVICTRL0_H_BLANKING_CNT)
#else
#define BF_ELCDIF_DVICTRL0_H_BLANKING_CNT(v)   (((v) << 0) & BM_ELCDIF_DVICTRL0_H_BLANKING_CNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the H_BLANKING_CNT field to a new value.
#define BW_ELCDIF_DVICTRL0_H_BLANKING_CNT(v)   BF_CS1(ELCDIF_DVICTRL0, H_BLANKING_CNT, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL0_H_ACTIVE_CNT(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_DVICTRL0_H_ACTIVE_CNT)
#else
#define BF_ELCDIF_DVICTRL0_H_ACTIVE_CNT(v)   (((v) << 16) & BM_ELCDIF_DVICTRL0_H_ACTIVE_CNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the H_ACTIVE_CNT field to a new value.
#define BW_ELCDIF_DVICTRL0_H_ACTIVE_CNT(v)   BF_CS1(ELCDIF_DVICTRL0, H_ACTIVE_CNT, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned F2_START_LINE : 10; //!< Vertical line number from which Field 2 begins.
        unsigned F1_END_LINE : 10; //!< Vertical line number at which Field1 ends.
        unsigned F1_START_LINE : 10; //!< Vertical line number from which Field 1 begins.
        unsigned RESERVED0 : 2; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL1_F2_START_LINE(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_DVICTRL1_F2_START_LINE)
#else
#define BF_ELCDIF_DVICTRL1_F2_START_LINE(v)   (((v) << 0) & BM_ELCDIF_DVICTRL1_F2_START_LINE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the F2_START_LINE field to a new value.
#define BW_ELCDIF_DVICTRL1_F2_START_LINE(v)   BF_CS1(ELCDIF_DVICTRL1, F2_START_LINE, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL1_F1_END_LINE(v)   ((((reg32_t) v) << 10) & BM_ELCDIF_DVICTRL1_F1_END_LINE)
#else
#define BF_ELCDIF_DVICTRL1_F1_END_LINE(v)   (((v) << 10) & BM_ELCDIF_DVICTRL1_F1_END_LINE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the F1_END_LINE field to a new value.
#define BW_ELCDIF_DVICTRL1_F1_END_LINE(v)   BF_CS1(ELCDIF_DVICTRL1, F1_END_LINE, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL1_F1_START_LINE(v)   ((((reg32_t) v) << 20) & BM_ELCDIF_DVICTRL1_F1_START_LINE)
#else
#define BF_ELCDIF_DVICTRL1_F1_START_LINE(v)   (((v) << 20) & BM_ELCDIF_DVICTRL1_F1_START_LINE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the F1_START_LINE field to a new value.
#define BW_ELCDIF_DVICTRL1_F1_START_LINE(v)   BF_CS1(ELCDIF_DVICTRL1, F1_START_LINE, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned V1_BLANK_END_LINE : 10; //!< Vertical line number in the beginning part of Field2 where first Vertical Blanking interval ends.
        unsigned V1_BLANK_START_LINE : 10; //!< Vertical line number towards the end of Field1 where first Vertical Blanking interval starts.
        unsigned F2_END_LINE : 10; //!< Vertical line number at which Field 2 ends.
        unsigned RESERVED0 : 2; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL2_V1_BLANK_END_LINE(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_DVICTRL2_V1_BLANK_END_LINE)
#else
#define BF_ELCDIF_DVICTRL2_V1_BLANK_END_LINE(v)   (((v) << 0) & BM_ELCDIF_DVICTRL2_V1_BLANK_END_LINE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the V1_BLANK_END_LINE field to a new value.
#define BW_ELCDIF_DVICTRL2_V1_BLANK_END_LINE(v)   BF_CS1(ELCDIF_DVICTRL2, V1_BLANK_END_LINE, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL2_V1_BLANK_START_LINE(v)   ((((reg32_t) v) << 10) & BM_ELCDIF_DVICTRL2_V1_BLANK_START_LINE)
#else
#define BF_ELCDIF_DVICTRL2_V1_BLANK_START_LINE(v)   (((v) << 10) & BM_ELCDIF_DVICTRL2_V1_BLANK_START_LINE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the V1_BLANK_START_LINE field to a new value.
#define BW_ELCDIF_DVICTRL2_V1_BLANK_START_LINE(v)   BF_CS1(ELCDIF_DVICTRL2, V1_BLANK_START_LINE, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL2_F2_END_LINE(v)   ((((reg32_t) v) << 20) & BM_ELCDIF_DVICTRL2_F2_END_LINE)
#else
#define BF_ELCDIF_DVICTRL2_F2_END_LINE(v)   (((v) << 20) & BM_ELCDIF_DVICTRL2_F2_END_LINE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the F2_END_LINE field to a new value.
#define BW_ELCDIF_DVICTRL2_F2_END_LINE(v)   BF_CS1(ELCDIF_DVICTRL2, F2_END_LINE, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned V_LINES_CNT : 10; //!< Total number of vertical lines per frame (generally 525 or 625)
        unsigned V2_BLANK_END_LINE : 10; //!< Vertical line number in the beginning part of Field1 where second Vertical Blanking interval ends.
        unsigned V2_BLANK_START_LINE : 10; //!< Vertical line number towards the end of Field2 where second Vertical Blanking interval starts.
        unsigned RESERVED0 : 2; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL3_V_LINES_CNT(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_DVICTRL3_V_LINES_CNT)
#else
#define BF_ELCDIF_DVICTRL3_V_LINES_CNT(v)   (((v) << 0) & BM_ELCDIF_DVICTRL3_V_LINES_CNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the V_LINES_CNT field to a new value.
#define BW_ELCDIF_DVICTRL3_V_LINES_CNT(v)   BF_CS1(ELCDIF_DVICTRL3, V_LINES_CNT, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL3_V2_BLANK_END_LINE(v)   ((((reg32_t) v) << 10) & BM_ELCDIF_DVICTRL3_V2_BLANK_END_LINE)
#else
#define BF_ELCDIF_DVICTRL3_V2_BLANK_END_LINE(v)   (((v) << 10) & BM_ELCDIF_DVICTRL3_V2_BLANK_END_LINE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the V2_BLANK_END_LINE field to a new value.
#define BW_ELCDIF_DVICTRL3_V2_BLANK_END_LINE(v)   BF_CS1(ELCDIF_DVICTRL3, V2_BLANK_END_LINE, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL3_V2_BLANK_START_LINE(v)   ((((reg32_t) v) << 20) & BM_ELCDIF_DVICTRL3_V2_BLANK_START_LINE)
#else
#define BF_ELCDIF_DVICTRL3_V2_BLANK_START_LINE(v)   (((v) << 20) & BM_ELCDIF_DVICTRL3_V2_BLANK_START_LINE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the V2_BLANK_START_LINE field to a new value.
#define BW_ELCDIF_DVICTRL3_V2_BLANK_START_LINE(v)   BF_CS1(ELCDIF_DVICTRL3, V2_BLANK_START_LINE, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned H_FILL_CNT : 8; //!< Number of active video samples that have to be filled with the filler data in the front and back portions of the active horizontal interval. Must be a multiple of 4. This field will have to be programmed if the input frame has less than 720 pixels per line.
        unsigned CR_FILL_VALUE : 8; //!< Value of CR component of filler data.
        unsigned CB_FILL_VALUE : 8; //!< Value of CB component of filler data
        unsigned Y_FILL_VALUE : 8; //!< Value of Y component of filler data
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL4_H_FILL_CNT(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_DVICTRL4_H_FILL_CNT)
#else
#define BF_ELCDIF_DVICTRL4_H_FILL_CNT(v)   (((v) << 0) & BM_ELCDIF_DVICTRL4_H_FILL_CNT)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the H_FILL_CNT field to a new value.
#define BW_ELCDIF_DVICTRL4_H_FILL_CNT(v)   BF_CS1(ELCDIF_DVICTRL4, H_FILL_CNT, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL4_CR_FILL_VALUE(v)   ((((reg32_t) v) << 8) & BM_ELCDIF_DVICTRL4_CR_FILL_VALUE)
#else
#define BF_ELCDIF_DVICTRL4_CR_FILL_VALUE(v)   (((v) << 8) & BM_ELCDIF_DVICTRL4_CR_FILL_VALUE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CR_FILL_VALUE field to a new value.
#define BW_ELCDIF_DVICTRL4_CR_FILL_VALUE(v)   BF_CS1(ELCDIF_DVICTRL4, CR_FILL_VALUE, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL4_CB_FILL_VALUE(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_DVICTRL4_CB_FILL_VALUE)
#else
#define BF_ELCDIF_DVICTRL4_CB_FILL_VALUE(v)   (((v) << 16) & BM_ELCDIF_DVICTRL4_CB_FILL_VALUE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CB_FILL_VALUE field to a new value.
#define BW_ELCDIF_DVICTRL4_CB_FILL_VALUE(v)   BF_CS1(ELCDIF_DVICTRL4, CB_FILL_VALUE, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DVICTRL4_Y_FILL_VALUE(v)   ((((reg32_t) v) << 24) & BM_ELCDIF_DVICTRL4_Y_FILL_VALUE)
#else
#define BF_ELCDIF_DVICTRL4_Y_FILL_VALUE(v)   (((v) << 24) & BM_ELCDIF_DVICTRL4_Y_FILL_VALUE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the Y_FILL_VALUE field to a new value.
#define BW_ELCDIF_DVICTRL4_Y_FILL_VALUE(v)   BF_CS1(ELCDIF_DVICTRL4, Y_FILL_VALUE, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned CSC_SUBSAMPLE_FILTER : 2; //!< This register describes the filtering and subsampling scheme to be performed on the chroma components in order to convert from YCbCr 4:4:4 to YCbCr 4:2:2 space. Note that the following descriptions apply individually to Cb and Cr.
        unsigned RESERVED0 : 14; //!< Reserved bits, write as 0.
        unsigned C0 : 10; //!< Two's complement red multiplier coefficient for Y
        unsigned RESERVED1 : 6; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF0_CSC_SUBSAMPLE_FILTER(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CSC_COEFF0_CSC_SUBSAMPLE_FILTER)
#else
#define BF_ELCDIF_CSC_COEFF0_CSC_SUBSAMPLE_FILTER(v)   (((v) << 0) & BM_ELCDIF_CSC_COEFF0_CSC_SUBSAMPLE_FILTER)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CSC_SUBSAMPLE_FILTER field to a new value.
#define BW_ELCDIF_CSC_COEFF0_CSC_SUBSAMPLE_FILTER(v)   BF_CS1(ELCDIF_CSC_COEFF0, CSC_SUBSAMPLE_FILTER, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF0_C0(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CSC_COEFF0_C0)
#else
#define BF_ELCDIF_CSC_COEFF0_C0(v)   (((v) << 16) & BM_ELCDIF_CSC_COEFF0_C0)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C0 field to a new value.
#define BW_ELCDIF_CSC_COEFF0_C0(v)   BF_CS1(ELCDIF_CSC_COEFF0, C0, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned C1 : 10; //!< Two's complement green multiplier coefficient for Y
        unsigned RESERVED0 : 6; //!< Reserved bits, write as 0.
        unsigned C2 : 10; //!< Two's complement blue multiplier coefficient for Y
        unsigned RESERVED1 : 6; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF1_C1(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CSC_COEFF1_C1)
#else
#define BF_ELCDIF_CSC_COEFF1_C1(v)   (((v) << 0) & BM_ELCDIF_CSC_COEFF1_C1)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C1 field to a new value.
#define BW_ELCDIF_CSC_COEFF1_C1(v)   BF_CS1(ELCDIF_CSC_COEFF1, C1, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF1_C2(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CSC_COEFF1_C2)
#else
#define BF_ELCDIF_CSC_COEFF1_C2(v)   (((v) << 16) & BM_ELCDIF_CSC_COEFF1_C2)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C2 field to a new value.
#define BW_ELCDIF_CSC_COEFF1_C2(v)   BF_CS1(ELCDIF_CSC_COEFF1, C2, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned C3 : 10; //!< Two's complement red multiplier coefficient for Cb
        unsigned RESERVED0 : 6; //!< Reserved bits, write as 0.
        unsigned C4 : 10; //!< Two's complement green multiplier coefficient for Cb
        unsigned RESERVED1 : 6; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF2_C3(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CSC_COEFF2_C3)
#else
#define BF_ELCDIF_CSC_COEFF2_C3(v)   (((v) << 0) & BM_ELCDIF_CSC_COEFF2_C3)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C3 field to a new value.
#define BW_ELCDIF_CSC_COEFF2_C3(v)   BF_CS1(ELCDIF_CSC_COEFF2, C3, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF2_C4(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CSC_COEFF2_C4)
#else
#define BF_ELCDIF_CSC_COEFF2_C4(v)   (((v) << 16) & BM_ELCDIF_CSC_COEFF2_C4)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C4 field to a new value.
#define BW_ELCDIF_CSC_COEFF2_C4(v)   BF_CS1(ELCDIF_CSC_COEFF2, C4, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned C5 : 10; //!< Two's complement blue multiplier coefficient for Cb
        unsigned RESERVED0 : 6; //!< Reserved bits, write as 0.
        unsigned C6 : 10; //!< Two's complement red multiplier coefficient for Cr
        unsigned RESERVED1 : 6; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF3_C5(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CSC_COEFF3_C5)
#else
#define BF_ELCDIF_CSC_COEFF3_C5(v)   (((v) << 0) & BM_ELCDIF_CSC_COEFF3_C5)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C5 field to a new value.
#define BW_ELCDIF_CSC_COEFF3_C5(v)   BF_CS1(ELCDIF_CSC_COEFF3, C5, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF3_C6(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CSC_COEFF3_C6)
#else
#define BF_ELCDIF_CSC_COEFF3_C6(v)   (((v) << 16) & BM_ELCDIF_CSC_COEFF3_C6)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C6 field to a new value.
#define BW_ELCDIF_CSC_COEFF3_C6(v)   BF_CS1(ELCDIF_CSC_COEFF3, C6, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned C7 : 10; //!< Two's complement green multiplier coefficient for Cr
        unsigned RESERVED0 : 6; //!< Reserved bits, write as 0.
        unsigned C8 : 10; //!< Two's complement blue multiplier coefficient for Cr
        unsigned RESERVED1 : 6; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF4_C7(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CSC_COEFF4_C7)
#else
#define BF_ELCDIF_CSC_COEFF4_C7(v)   (((v) << 0) & BM_ELCDIF_CSC_COEFF4_C7)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C7 field to a new value.
#define BW_ELCDIF_CSC_COEFF4_C7(v)   BF_CS1(ELCDIF_CSC_COEFF4, C7, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_COEFF4_C8(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CSC_COEFF4_C8)
#else
#define BF_ELCDIF_CSC_COEFF4_C8(v)   (((v) << 16) & BM_ELCDIF_CSC_COEFF4_C8)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the C8 field to a new value.
#define BW_ELCDIF_CSC_COEFF4_C8(v)   BF_CS1(ELCDIF_CSC_COEFF4, C8, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned Y_OFFSET : 9; //!< Two's complement offset for the Y component
        unsigned RESERVED0 : 7; //!< Reserved bits, write as 0.
        unsigned CBCR_OFFSET : 9; //!< Two's complement offset for the Cb and Cr components
        unsigned RESERVED1 : 7; //!< Reserved bits, write as 0.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_OFFSET_Y_OFFSET(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CSC_OFFSET_Y_OFFSET)
#else
#define BF_ELCDIF_CSC_OFFSET_Y_OFFSET(v)   (((v) << 0) & BM_ELCDIF_CSC_OFFSET_Y_OFFSET)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the Y_OFFSET field to a new value.
#define BW_ELCDIF_CSC_OFFSET_Y_OFFSET(v)   BF_CS1(ELCDIF_CSC_OFFSET, Y_OFFSET, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_OFFSET_CBCR_OFFSET(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CSC_OFFSET_CBCR_OFFSET)
#else
#define BF_ELCDIF_CSC_OFFSET_CBCR_OFFSET(v)   (((v) << 16) & BM_ELCDIF_CSC_OFFSET_CBCR_OFFSET)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CBCR_OFFSET field to a new value.
#define BW_ELCDIF_CSC_OFFSET_CBCR_OFFSET(v)   BF_CS1(ELCDIF_CSC_OFFSET, CBCR_OFFSET, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned Y_MAX : 8; //!< Upper limit of Y after RGB to 4:2:2 YCbCr conversion
        unsigned Y_MIN : 8; //!< Lower limit of Y after RGB to 4:2:2 YCbCr conversion
        unsigned CBCR_MAX : 8; //!< Upper limit of Cb and Cr after RGB to 4:2:2 YCbCr conversion
        unsigned CBCR_MIN : 8; //!< Lower limit of Cb and Cr after RGB to 4:2:2 YCbCr conversion
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_LIMIT_Y_MAX(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CSC_LIMIT_Y_MAX)
#else
#define BF_ELCDIF_CSC_LIMIT_Y_MAX(v)   (((v) << 0) & BM_ELCDIF_CSC_LIMIT_Y_MAX)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the Y_MAX field to a new value.
#define BW_ELCDIF_CSC_LIMIT_Y_MAX(v)   BF_CS1(ELCDIF_CSC_LIMIT, Y_MAX, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_LIMIT_Y_MIN(v)   ((((reg32_t) v) << 8) & BM_ELCDIF_CSC_LIMIT_Y_MIN)
#else
#define BF_ELCDIF_CSC_LIMIT_Y_MIN(v)   (((v) << 8) & BM_ELCDIF_CSC_LIMIT_Y_MIN)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the Y_MIN field to a new value.
#define BW_ELCDIF_CSC_LIMIT_Y_MIN(v)   BF_CS1(ELCDIF_CSC_LIMIT, Y_MIN, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_LIMIT_CBCR_MAX(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_CSC_LIMIT_CBCR_MAX)
#else
#define BF_ELCDIF_CSC_LIMIT_CBCR_MAX(v)   (((v) << 16) & BM_ELCDIF_CSC_LIMIT_CBCR_MAX)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CBCR_MAX field to a new value.
#define BW_ELCDIF_CSC_LIMIT_CBCR_MAX(v)   BF_CS1(ELCDIF_CSC_LIMIT, CBCR_MAX, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CSC_LIMIT_CBCR_MIN(v)   ((((reg32_t) v) << 24) & BM_ELCDIF_CSC_LIMIT_CBCR_MIN)
#else
#define BF_ELCDIF_CSC_LIMIT_CBCR_MIN(v)   (((v) << 24) & BM_ELCDIF_CSC_LIMIT_CBCR_MIN)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CBCR_MIN field to a new value.
#define BW_ELCDIF_CSC_LIMIT_CBCR_MIN(v)   BF_CS1(ELCDIF_CSC_LIMIT, CBCR_MIN, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned DATA_ZERO : 8; //!< Byte 0 (least significant byte) of data written to eLCDIF by the DMA or the ARM platform.
        unsigned DATA_ONE : 8; //!< Byte 1 of data written to eLCDIF by the DMA or the ARM platform.
        unsigned DATA_TWO : 8; //!< Byte 2 of data written to eLCDIF by the DMA or the ARM platform.
        unsigned DATA_THREE : 8; //!< Byte 3 (most significant byte) of data written to LCDIF by the DMA or the ARM platform.
    } B;
} hw_elcdif_data_t;
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DATA_DATA_ZERO(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_DATA_DATA_ZERO)
#else
#define BF_ELCDIF_DATA_DATA_ZERO(v)   (((v) << 0) & BM_ELCDIF_DATA_DATA_ZERO)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the DATA_ZERO field to a new value.
#define BW_ELCDIF_DATA_DATA_ZERO(v)   BF_CS1(ELCDIF_DATA, DATA_ZERO, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DATA_DATA_ONE(v)   ((((reg32_t) v) << 8) & BM_ELCDIF_DATA_DATA_ONE)
#else
#define BF_ELCDIF_DATA_DATA_ONE(v)   (((v) << 8) & BM_ELCDIF_DATA_DATA_ONE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the DATA_ONE field to a new value.
#define BW_ELCDIF_DATA_DATA_ONE(v)   BF_CS1(ELCDIF_DATA, DATA_ONE, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DATA_DATA_TWO(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_DATA_DATA_TWO)
#else
#define BF_ELCDIF_DATA_DATA_TWO(v)   (((v) << 16) & BM_ELCDIF_DATA_DATA_TWO)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the DATA_TWO field to a new value.
#define BW_ELCDIF_DATA_DATA_TWO(v)   BF_CS1(ELCDIF_DATA, DATA_TWO, v)
#endif
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_DATA_DATA_THREE(v)   ((((reg32_t) v) << 24) & BM_ELCDIF_DATA_DATA_THREE)
#else
#define BF_ELCDIF_DATA_DATA_THREE(v)   (((v) << 24) & BM_ELCDIF_DATA_DATA_THREE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the DATA_THREE field to a new value.
#define BW_ELCDIF_DATA_DATA_THREE(v)   BF_CS1(ELCDIF_DATA, DATA_THREE, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned ADDR : 32; //!< Virtual address at which bus master error occurred.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_BM_ERROR_STAT_ADDR(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_BM_ERROR_STAT_ADDR)
#else
#define BF_ELCDIF_BM_ERROR_STAT_ADDR(v)   (((v) << 0) & BM_ELCDIF_BM_ERROR_STAT_ADDR)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the ADDR field to a new value.
#define BW_ELCDIF_BM_ERROR_STAT_ADDR(v)   BF_CS1(ELCDIF_BM_ERROR_STAT, ADDR, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned CRC_VALUE : 32; //!< Calculated CRC value.
    } B;
...
#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_CRC_STAT_CRC_VALUE(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_CRC_STAT_CRC_VALUE)
#else
#define BF_ELCDIF_CRC_STAT_CRC_VALUE(v)   (((v) << 0) & BM_ELCDIF_CRC_STAT_CRC_VALUE)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the CRC_VALUE field to a new value.
#define BW_ELCDIF_CRC_STAT_CRC_VALUE(v)   BF_CS1(ELCDIF_CRC_STAT, CRC_VALUE, v)
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned LFIFO_COUNT : 9; //!< Read only view of the current count in Latency buffer (LFIFO).
        unsigned RESERVED0 : 15; //!< Reserved bits. Write as 0.
        unsigned DVI_CURRENT_FIELD : 1; //!< Read only view of the current field being transmitted. DVI_CURRENT_FIELD = 0 means field 1. DVI_CURRENT_FIELD = 1 means field 2.
        unsigned BUSY : 1; //!< Read only view of the input busy signal from the external LCD controller.
        unsigned TXFIFO_EMPTY : 1; //!< Read only view of the signal that indicates that LCD write dapatath FIFO is empty, will be generally used in the read mode of the LCD interface.
        unsigned TXFIFO_FULL : 1; //!< Read only view of the signal that indicates that LCD write datapath FIFO is full, will be generally used in the write mode of the LCD interface.
        unsigned LFIFO_EMPTY : 1; //!< Read only view of the signal that indicates that LCD read dapatath FIFO is empty, will be generally used in the read mode of the LCD interface.
        unsigned LFIFO_FULL : 1; //!< Read only view of the signal that indicates that LCD read datapath FIFO is full, will be generally used in the write mode of the LCD interface.
        unsigned DMA_REQ : 1; //!< Reflects the current state of the DMA Request line for the eLCDIF. The DMA Request line toggles for each new request.
        unsigned PRESENT : 1; //!< 0: eLCDIF not present on this product 1: eLCDIF is present.
    } B;
} hw_elcdif_stat_t;
#endif
...
#define BP_ELCDIF_STAT_TXFIFO_EMPTY      (26)
#define BM_ELCDIF_STAT_TXFIFO_EMPTY      (0x04000000)

/* --- Register HW_ELCDIF_STAT, field TXFIFO_FULL[27:27] (RO)
 *
...
typedef union
{
    ...
    struct
    {
        unsigned STEP : 16; //!< Fixed read-only value reflecting the stepping of RTL version.
        unsigned MINOR : 8; //!< Fixed read-only value reflecting the MINOR field of RTL version.
        unsigned MAJOR : 8; //!< Fixed read-only value reflecting the MAJOR field of RTL version.
    } B;
} hw_elcdif_version_t;
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned MST_WORDS : 4; //!< Read only view of the current bursts issued by the AXI bus master.
        unsigned MST_OUTSTANDING_REQS : 5; //!< Read only view of the current outstanding requests issued by the AXI bus master.
        unsigned MST_AVALID : 1; //!< Read only view of the mst_avalid signal issued by the AXI bus master.
        unsigned CUR_REQ_STATE : 2; //!< Read only view of the request state machine.
        unsigned PXP_B1_DONE : 1; //!< Buffer1 done signal issued by eLCDIF.
        unsigned PXP_LCDIF_B1_READY : 1; //!< Buffer1 ready signal issued by ePXP.
        unsigned PXP_B0_DONE : 1; //!< Buffer0 done signal issued by eLCDIF.
        unsigned PXP_LCDIF_B0_READY : 1; //!< Buffer0 ready signal issued by ePXP.
        unsigned CUR_STATE : 7; //!< Read only view of the current state machine state in the current mode of operation.
        unsigned EMPTY_WORD : 1; //!< Indicates that the current word is empty.
        unsigned CUR_FRAME_TX : 1; //!< This bit is 1 for the time the current frame is being transmitted in the VSYNC mode. Useful for VSYNC mode debug.
        unsigned VSYNC : 1; //!< Read only view of VSYNC signal.
        unsigned HSYNC : 1; //!< Read only view of HSYNC signal.
        unsigned ENABLE : 1; //!< Read only view of ENABLE signal.
        unsigned DMACMDKICK : 1; //!< Read only view of the DMA command kick signal.
        unsigned SYNC_SIGNALS_ON_REG : 1; //!< Read only view of internal sync_signals_on_reg signal.
        unsigned WAIT_FOR_VSYNC_EDGE_OUT : 1; //!< Read only view of WAIT_FOR_VSYNC_EDGE bit in the VSYNC mode after it comes out of the TXFIFO.
        unsigned STREAMING_END_DETECTED : 1; //!< Read only view of the DOTCLK_MODE or DVI_MODE bit going from 1 to 0.
    } B;
} hw_elcdif_debug0_t;
#endif
...
/* --- Register HW_ELCDIF_DEBUG0, field MST_AVALID[9:9] (RO)
 *
 * Read only view of the mst_avalid signal issued by the AXI bus master.
 */

#define BP_ELCDIF_DEBUG0_MST_AVALID      (9)
...
/* --- Register HW_ELCDIF_DEBUG0, field PXP_B1_DONE[12:12] (RO)
 *
 * Buffer1 done signal issued by eLCDIF.
 */

#define BP_ELCDIF_DEBUG0_PXP_B1_DONE      (12)
#define BM_ELCDIF_DEBUG0_PXP_B1_DONE      (0x00001000)

/* --- Register HW_ELCDIF_DEBUG0, field PXP_LCDIF_B1_READY[13:13] (RO)
 *
 * Buffer1 ready signal issued by ePXP.
 */

#define BP_ELCDIF_DEBUG0_PXP_LCDIF_B1_READY      (13)
#define BM_ELCDIF_DEBUG0_PXP_LCDIF_B1_READY      (0x00002000)

/* --- Register HW_ELCDIF_DEBUG0, field PXP_B0_DONE[14:14] (RO)
 *
 * Buffer0 done signal issued by eLCDIF.
 */

#define BP_ELCDIF_DEBUG0_PXP_B0_DONE      (14)
#define BM_ELCDIF_DEBUG0_PXP_B0_DONE      (0x00004000)

/* --- Register HW_ELCDIF_DEBUG0, field PXP_LCDIF_B0_READY[15:15] (RO)
 *
 * Buffer0 ready signal issued by ePXP.
 */

#define BP_ELCDIF_DEBUG0_PXP_LCDIF_B0_READY      (15)
#define BM_ELCDIF_DEBUG0_PXP_LCDIF_B0_READY      (0x00008000)
...
typedef union
{
    ...
    struct
    {
        unsigned V_DATA_COUNT : 16; //!< Read only view of the current state of the vertical data counter.
        unsigned H_DATA_COUNT : 16; //!< Read only view of the current state of the horizontal data counter.
    } B;
} hw_elcdif_debug1_t;
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned MST_ADDRESS : 32; //!< Read only view of the current address issued by the AXI bus master.
    } B;
} hw_elcdif_debug2_t;
#endif
...
typedef union
{
    ...
    struct
    {
        unsigned PANIC : 9; //!< This value should be set to a value of pixels, from 0 to 511. When the number of pixels in the input pixel FIFO is LESS than this value, the panic control output will be raised. This signal can be used to raise the access eLCDIF's access priority.
        unsigned RESERVED0 : 7; //!< Reserved bits. Write as 0.
        unsigned FASTCLOCK : 9; //!< This value should be set to a value of pixels, from 0 to 511. When the number of pixels in the input pixel FIFO is LESS than this value, the fast clock control output will be raised. This signal can be used to reduce the system bus clock frequency to save power during horizontal/vertical blanking intervals. This value should also be programmed to a value that is greater than the "PANIC" threshold value. This will allow a faster clock to recover the number of pixels in the FIFO before a "panic" level is encountered.
        unsigned RESERVED1 : 7; //!< Reserved bits. Write as 0.
    } B;
} hw_elcdif_thres_t;
#endif
...
#define BP_ELCDIF_THRES_PANIC      (0)
#define BM_ELCDIF_THRES_PANIC      (0x000001ff)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_THRES_PANIC(v)   ((((reg32_t) v) << 0) & BM_ELCDIF_THRES_PANIC)
#else
#define BF_ELCDIF_THRES_PANIC(v)   (((v) << 0) & BM_ELCDIF_THRES_PANIC)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the PANIC field to a new value.
#define BW_ELCDIF_THRES_PANIC(v)   BF_CS1(ELCDIF_THRES, PANIC, v)
#endif
...
#define BP_ELCDIF_THRES_FASTCLOCK      (16)
#define BM_ELCDIF_THRES_FASTCLOCK      (0x01ff0000)

#ifndef __LANGUAGE_ASM__
#define BF_ELCDIF_THRES_FASTCLOCK(v)   ((((reg32_t) v) << 16) & BM_ELCDIF_THRES_FASTCLOCK)
#else
#define BF_ELCDIF_THRES_FASTCLOCK(v)   (((v) << 16) & BM_ELCDIF_THRES_FASTCLOCK)
#endif
#ifndef __LANGUAGE_ASM__
//! @brief Set the FASTCLOCK field to a new value.
#define BW_ELCDIF_THRES_FASTCLOCK(v)   BF_CS1(ELCDIF_THRES, FASTCLOCK, v)
#endif
