...
{
    struct _apbh_dma_gpmi1_t*	  nxt;
    hw_apbh_chn_cmd_t             cmd;              // Contents for the HW_ABBH_CHn_CMD register
    void*                         bar;              // Address of a buffer with additional parameters
    hw_gpmi_ctrl0_t       gpmi_ctrl0;       // For HW_GPMI_CTRL0 register
    hw_gpmi_compare_t     gpmi_compare;     // For HW_GPMI_COMPARE register
    hw_gpmi_eccctrl_t     gpmi_eccctrl;     // For HW_GPMI_ECCCTRL register
} apbh_dma_gpmi3_t;
...
public:
    ...
    virtual dma_cmd_t * getFirstDescriptor()=0;
...
public:
    ...
    //! \brief Returns the address of the first DMA descriptor.
    virtual dma_cmd_t * getFirstDescriptor();
...
public:
    ...
    //! \brief Update buffers.
    //!
    //! \note Because of how whether or not the #sendAuxData descriptor is used depends on the
...
public:
    ...
    //! \brief Just start the DMA and return immediately.
    virtual RtStatus_t start(uint32_t chainSize=0);
    
    //! \brief Start DMA and wait for it to complete.
    virtual RtStatus_t startAndWait(uint32_t timeoutMicroseconds, uint32_t chainSize=0);
...
{
public:
    dma_cmd_t * m_dmaStart; //!< The first DMA descriptor.
    ...
    //! \brief Constructor.
    inline WrappedSequence(unsigned chipSelect, dma_cmd_t * dma, uint16_t theMask=0)
    ...
    inline dma_cmd_t * getDmaStart() { return m_dmaStart; }
    
    //! \brief Change the DMA start descriptor.
    inline void setDmaStart(dma_cmd_t * theDma) { m_dmaStart = theDma; }
    ...
    //! \brief Returns the address of the first DMA descriptor.
    inline virtual dma_cmd_t * getFirstDescriptor() { return m_dmaStart; }
...
public:
    ...
    inline operator dma_cmd_t * () { return getFirstDescriptor(); }
};
...
public:
    ...
    //! \brief Constructor.
    inline ReadEccData(unsigned chipSelect, uint8_t command1, uint8_t * addressBytes, unsigned addressByteCount, uint8_t command2, void * dataBuffer, void * auxBuffer, uint32_t readSize, const NandEccDescriptor_t & ecc, uint32_t eccMask)
    {
        init(chipSelect, command1, addressBytes, addressByteCount, command2, dataBuffer, auxBuffer, readSize, ecc, eccMask);
    }
    
    //! \brief Initializer.
    void init(unsigned chipSelect, uint8_t command1, uint8_t * addressBytes, unsigned addressByteCount, uint8_t command2, void * dataBuffer, void * auxBuffer, uint32_t readSize, const NandEccDescriptor_t & ecc, uint32_t eccMask);
    ...
    void setBuffers(void * dataBuffer, void * auxBuffer, uint32_t readSize, const NandEccDescriptor_t & ecc, uint32_t eccMask);
    //@}
...
public:
    ...
    //! \brief Constructor.
    inline WriteEccData(unsigned chipSelect, uint8_t command1, uint8_t * addressBytes, unsigned addressByteCount, uint8_t command2, const void * dataBuffer, const void * auxBuffer, uint32_t sendSize, uint32_t dataSize, uint32_t leftoverSize, const NandEccDescriptor_t & ecc, uint32_t eccMask)
    {
    ...
    //! \brief Initializer.
    void init(unsigned chipSelect, uint8_t command1, uint8_t * addressBytes, unsigned addressByteCount, uint8_t command2, const void * dataBuffer, const void * auxBuffer, uint32_t sendSize, uint32_t dataSize, uint32_t leftoverSize, const NandEccDescriptor_t & ecc, uint32_t eccMask);
    ...
    void setBuffers(const void * dataBuffer, const void * auxBuffer, uint32_t sendSize, uint32_t dataSize, uint32_t leftoverSize, const NandEccDescriptor_t & ecc, uint32_t eccMask);
    //@}
