...
extern byte siiReadSegmentBlockEDID(byte SlaveAddr, byte Segment, byte Offset, byte * Buffer,
                                    byte Length);
...
{
    byte data = I2CReadByte(TX_SLAVE_ADDR, RegOffset);
    return data;
}
...
{
//    byte data = I2CReadByte(TX_SLAVE_ADDR, RegOffset);
//    TPI_DEBUG_PRINT(("reg 0x%02x:data from 0x%x ==> ", RegOffset, data));
    I2CWriteByte(TX_SLAVE_ADDR, RegOffset, Data);
//    data = I2CReadByte(TX_SLAVE_ADDR, RegOffset);
//    TPI_DEBUG_PRINT(("0x%x\n ", data));
}
...
{
//    int i = 0;
//    byte *dataBack = malloc(NBytes);
    I2CWriteBlock(TX_SLAVE_ADDR, TPI_Offset, NBytes, pData);
#if 0
    I2CReadBlock(TX_SLAVE_ADDR, TPI_Offset, NBytes, dataBack);
    TPI_DEBUG_PRINT(("TPI BLOCK: "));

    for (i = 0; i < NBytes; i++)
        TPI_DEBUG_PRINT(("0x%x ", dataBack[i]));
...
void TxHW_Reset(void)
{
    ...
    gpio_write_data(GPIO_PORT5, 0, 0x0);
    DelayMS(TX_HW_RESET_PERIOD);
    gpio_write_data(GPIO_PORT5, 0, 0x1);
    DelayMS(TX_HW_RESET_PERIOD);
    TXHAL_InitPostReset();
}
...
// Globals: EDID data
//------------------------------------------------------------------------------
#if (CONF__TPI_EDID_PRINT == DEBUG_ENABLE)
...
byte DoEdidRead(void)
{
    ...
    if (g_edid.edidDataValid == FALSE) {
        ...
        if (GetDDC_Access(&SysCtrlReg)) {
            ...
            ReadSegmentBlockEDID(0, EDID_BLOCK_0_OFFSET, EDID_BLOCK_SIZE, g_CommData);  // read next 128 bytes of EDID ROM

            Result = ParseEDID(g_CommData, &NumOfExtensions);

            if (Result != EDID_OK) {
                if (Result == EDID_NO_861_EXTENSIONS) {
                    TPI_EDID_PRINT(("EDID -> No 861 Extensions\n"));
                    g_edid.HDMI_Sink = FALSE;
                    ...
                    TPI_EDID_PRINT(("EDID -> Parse FAILED\n"));
                    g_edid.HDMI_Sink = TRUE;
                    g_edid.YCbCr_4_4_4 = FALSE;
                    g_edid.YCbCr_4_2_2 = FALSE;
                    g_edid.CEC_A_B = 0x00;
            ...
            {
                TPI_EDID_PRINT(("EDID -> DDC bus release failed\n"));
                return EDID_DDC_BUS_RELEASE_FAILURE;
            }
        } else {
            TPI_EDID_PRINT(("EDID -> DDC bus request failed\n"));
            g_edid.HDMI_Sink = TRUE;
            g_edid.YCbCr_4_4_4 = FALSE;
            g_edid.YCbCr_4_2_2 = FALSE;
            g_edid.CEC_A_B = 0x00;
        ...
        TPI_EDID_PRINT(("EDID -> g_edid.HDMI_Sink = %d\n", (int)g_edid.HDMI_Sink));
        TPI_EDID_PRINT(("EDID -> g_edid.YCbCr_4_4_4 = %d\n", (int)g_edid.YCbCr_4_4_4));
        TPI_EDID_PRINT(("EDID -> g_edid.YCbCr_4_2_2 = %d\n", (int)g_edid.YCbCr_4_2_2));
        TPI_EDID_PRINT(("EDID -> g_edid.CEC_A_B = 0x%x\n", (int)g_edid.CEC_A_B));
        TPI_EDID_PRINT(("EDID -> g_edid.CEC_C_D = 0x%x\n", (int)g_edid.CEC_C_D));
        ...
        if (InterruptStatusImage & HDCP_CHANGE_EVENT) {
            ...
            if (NewLinkProtectionLevel != g_hdcp.HDCP_LinkProtectionLevel) {
                ...
                case LOCAL_LINK_PROTECTION_SECURE:
                    ...
                    if (IsHDMI_Sink()) {
                        ReadModifyWriteTPI(TPI_AUDIO_INTERFACE_REG, AUDIO_MUTE_MASK,
                                           AUDIO_MUTE_NORMAL);
                    }
...
byte StartTPI(void)
{
    ...
    TPI_TRACE_PRINT(("Chip ID is 0x%04X\n", (int)wID));
...
#endif
    {
        InterruptStatus = HOT_PLUG_EVENT;   //ReadByteTPI(TPI_INTERRUPT_STATUS_REG);
...
void siHdmiTx_VideoSel(byte vmode)
{
    ...
    siHdmiTx.ColorSpace = HDMIRGB;
    siHdmiTx.ColorDepth = VMD_COLOR_DEPTH_8BIT;
    siHdmiTx.SyncMode = EXTERNAL_HSVSDE;

    switch (vmode) {
    case SHDMI_480I60_4X3:
        siHdmiTx.VIC = 6;
