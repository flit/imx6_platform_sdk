...
int encoder_allocate_framebuffer(struct encode *enc)
{
    ...
    for (i = 0; i < fbcount; i++) {
        pfbpool[i] =
            framebuf_alloc(enc->cmdl->format, MODE420, (enc->enc_picwidth + 15) & ~15,
...
static int encoder_start(struct encode *enc)
{
    ...
    while (1) {
        ...
        if (outinfo.mbInfo.enable && outinfo.mbInfo.size && outinfo.mbInfo.addr) {
            SaveEncMbInfo(outinfo.mbInfo.addr, outinfo.mbInfo.size, encop.picWidth / 16, frame_id);
        }

        if (outinfo.mvInfo.enable && outinfo.mvInfo.size && outinfo.mvInfo.addr) {
            SaveEncMvInfo(outinfo.mvInfo.addr, outinfo.mvInfo.size, encop.picWidth / 16, frame_id);
        }

        if (outinfo.sliceInfo.enable && outinfo.sliceInfo.size && outinfo.sliceInfo.addr) {
            SaveEncSliceInfo(outinfo.sliceInfo.addr, outinfo.sliceInfo.size, frame_id);
        }
...
int encode_test(void *arg)
{
    ...
    enc->virt_bsbuf_addr = IOGetMem(&mem_desc);
